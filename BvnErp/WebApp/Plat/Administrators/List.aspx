    ...
    <script type="text/javascript">
        ...
        var btnformatter = function (val, rec) {
            var arry = [
                ...
                '<button style="cursor:pointer;" onclick="mySubordinates(\'' + rec.ID + '\');">分配员工</button>',
                '<button style="cursor:pointer;" onclick="myClients(\'' + rec.ID + '\');">分配客户</button>',
                //'<button style="cursor:pointer;" onclick="myVenders(\'' + rec.ID + '\');">分配公司</button>',
                //'<button style="cursor:pointer;" onclick="myContacts(\'' + rec.ID + '\');">分配联系人</button>',
        ...
        var myClients = function (id) {
            top.$.myWindow({ url: '/wss/AllotClient/List.aspx?pid=' + id }).open();
        };
...
<body>
    <div class="easyui-panel"  title="管理员列表" data-options="fit:true">
        <div id="toolbar">
            <a id="btnAdd" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-add'" onclick="Search()">添加</a>
