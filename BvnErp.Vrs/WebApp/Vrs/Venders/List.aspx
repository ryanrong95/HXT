<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="WebApp.Vrs.Venders.List" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title></title>
    <uc:EasyUI runat="server" />
    <style>
        .ven_type {
            height: 40px;
        }

            .ven_type button {
                margin-top: 8px;
            }
    </style>
    <script>
        /* 每个需要颗粒化的页面都需要指定 menu ，否则不会写入菜单和颗粒化*/
        gvSettings.fatherMenu = '供应商管理.';
        gvSettings.menu = '供应商管理';
        gvSettings.summary = '';

    </script>
    <script>
        var param = "";
        var getAjaxData = function () {
            var data = {
                action: 'data',
                param: param
            };
            return data;
        };
        var btnformatter = function (val, rec) {
            var arry = [];
            arry.push('<button style="cursor:pointer;" onclick="contact(\'' + rec.ID + '\');">联系人</button>');
            arry.push('<button style="cursor:pointer;" onclick="edit(\'' + rec.ID + '\');">编辑</button>'
                , '<button style="cursor:pointer;" onclick="del(\'' + rec.ID + '\');">删除</button>')
            return arry.join('|');
        };
        $(function () {
            $('#t_vender').bvgrid({ queryParams: getAjaxData() });
        });
        function search(type) {
            param = type;
            var data = getAjaxData();
            $("#t_vender").bvgrid('search', data);
        }
        var edit = function (id) {
            top.$.myWindow({
                url: location.pathname.replace(/List.aspx/ig, 'Edit.aspx') + '?id=' + id, onClose: function () {
                    $("#t_vender").bvgrid('reload');
                }
            }).open();
        };
        var del = function (id) {
            $.messager.confirm('删除提示', '确定要删除吗?', function (r) {
                if (r) {
                    $.post("?action=del", { id: id }, function (data) {
                        if (data) {
                            $.messager.alert('提示', '删除成功');
                            $("#t_vender").bvgrid('reload');
                        }
                    })
                }
            });
        }
        function add() {
            top.$.myWindow({
                url: location.pathname.replace(/List.aspx/ig, 'Edit.aspx'),
                onClose: function () {
                    $("#t_vender").bvgrid('reload');
                }
            }).open();
        }
        var editStatus = function (id, status) {
            $.post("?action=editStatus", { id: id, status: status }, function (data) {
                if (data) {
                    if (data) {
                        $("#t_vender").bvgrid('reload');
                    }
                }
            })
        }
        var contact = function (id) {
            top.$.myWindow({
                url: location.pathname.replace(/Venders\/List.aspx/ig, '/Contacts/List.aspx') + "?id=" + id
            }).open();
        }
    </script>
</head>
<body>
    <div class="easyui-panel" data-options="border:true,fit:true,closable:true,onClose:function(){$.myWindow.close();}" title="供应商/制造商/代理商管理">
        <div class="ven_type">
            <button onclick="add()">添加</button>
            <button onclick="search('')">全部</button>
            <%
                foreach (var item in Enum.GetValues(typeof(NtErp.Vrs.Services.Enums.ComapnyType)).Cast<NtErp.Vrs.Services.Enums.ComapnyType>())
                {
            %>
            <button onclick="search(<%=(int)item %>)"><%=item.ToString() %></button>
            <%
                }
            %>
        </div>

        <table id="t_vender" class="easyui-datagrid">
            <thead>
                <tr>
                    <th data-options="field:'Name',width:100,align:'center'">名称</th>
                    <th data-options="field:'Type',width:80,align:'center'">类别</th>
                    <th data-options="field:'Grade',width:80,align:'center'">级别</th>
                    <th data-options="field:'Address',width:150,align:'center'">公司地址</th>
                    <th data-options="field:'Status',width:150,align:'center'">状态</th>
                    <th data-options="field:'Btns',formatter:btnformatter,align:'center',width:200">操作</th>
                </tr>
            </thead>
        </table>

    </div>
</body>
</html>
