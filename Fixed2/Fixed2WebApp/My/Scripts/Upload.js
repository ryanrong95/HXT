...
function upload(val_obj) {
    ...
    var inputfile = document.getElementById(obj.el);
    if (!inputfile) {
        console.error(obj.el + 'DOM元素没有找到');
        return;
    }
    init();
    var dz = document.getElementById(obj.el).parentNode.getElementsByClassName(obj.el + '1')[0];
    if (dz.className.length > 0)
        dz.className = dz.className + " daoUpload";
    else
        dz.className = "daoUpload";
    dz.setAttribute('dropEffect', 'link');
    var getParentForm = function (gname) {
        var nodename = gname.parentNode.nodeName;
        if (nodename != 'FORM') {
            return getParentForm(gname.parentNode);
        }
        return gname.parentNode;
    };
    ...
    var formtag = getParentForm(document.getElementById(obj.el));
    formtag.onsubmit = function () {
        ...
        if (uploadFiles.fail.length > 0) {
            ...
            //alert(document.getElementById(obj.el).value);
            alert(formtag.value);
            return true;
            alert("下一步添加数据！");
        }
        ...
        setTimeout(function () {
            ...
            var ls_list = function () {
                ...
                for (var i = 0; i < files.length; i++) {
                    formData.append(obj.fileName, files[i]);
                }
    ...
    function uploadFile(formData, val) {
        ...
        if (xhr.status == 200) {
            ...
            document.getElementById(obj.el).value = uploadFiles.callArr.join(','); /*hidden*/
            alert(document.getElementById(obj.el).value);
    ...
    function init() {
        /* input type=file 的替换容器 */
        var container = document.createElement('div');
        container.style.width = inputfile.style.width;
        container.style.height = inputfile.style.height;
        container.className = inputfile.id + '1',
        container.id = inputfile.id + '1',
        container.innerHTML = "";
        inputfile.parentNode.insertBefore(container, inputfile);
        /* input type=file 替换，使url 赋值value */
        var urlinput = document.createElement('input');
        urlinput.id = inputfile.id;
        if (inputfile.name == undefined || inputfile.name == "") {
            urlinput.name = inputfile.id;
        }
        else {
            urlinput.name = inputfile.name;
        }
        urlinput.setAttribute('type', 'hidden');
        inputfile.parentNode.insertBefore(urlinput, inputfile);
        ...
        container.appendChild(fileOption);
