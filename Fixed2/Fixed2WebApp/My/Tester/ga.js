...
(function ($) { /*默认值*/
    var defaults = {
        menu: '',
        /*菜单名称*/
        rawurl: document.URL.replace(/\?.*?$/ig, '').toLowerCase(),
        /*当前页面url*/
        actionurl: '/ga.js.aspx?action=roleunites',
        /*写入action*/
        readurl: '/ga.js.aspx?action=readunites',
        /*配置读取action*/
    ...
    $.extend({
        roleUnites: function (options) {
            options = options || {};
            var obj = new Object(); /*项*/
            obj.options = $.extend({}, defaults, options); /*标签集合*/
            obj.labels = $('[' + uniteLabel.names.join('],[') + ']'); /*roleUnite 对象集合*/
            /*obj.options.rawurl=getRelativeUrl(obj.options.rawurl);*/
            obj.sets = getSets(obj.options.menu, obj.options.rawurl, obj.labels); /*读取入库*/
            obj.enter = function () { /*验证 menu 必需项*/
                ...
                $.ajax({
                    ...
                    data: {
                        menu: obj.options.menu,
                        url: obj.options.rawurl,
                        sets: JSON.stringify(obj.sets)
                    },
                    dataType: 'json',
                    success: function (data) {
                        console.log(obj.options.menu + ' unites write success'); /*callback*/
            ...
            obj.read = function () {
                var arry = eval('([{"ID":"RU0000000003","Type":2,"Menu":"开发手记","Name":"btn","Title":"列表项操作按钮组","Url":"/plat/devlops/list.aspx","CreateDate":"2018-07-26T10:02:49.05"}])');
                if (arry && arry.length > 0) {
                    $.each(arry, function (index, item) {
                        if (item.Type == 1) {
                            ...
                            gname.remove(); /*如果是在 table 上加的g_name标签，则隐藏整个table*/
                        }
                    });
                }
                console.log(obj.options.menu + ' unites read success'); /*callback*/
    ...
    var getSets = function (menu, url, labels) {
        ...
        labels.each(function () {
            ...
            arry.push({
                ...
                Name: vname,
                /*标识名称*/
                Title: $(this).attr(uniteLabel.title),
                /*标识title*/
                Url: url,
                /*当前页面url*/
            });
...
})(jQuery); /**获取当前相对路径*比如 http://www.xx.com/public/item.aspx?t=110 结果为：/public/item.aspx*/
...
$(function () {
    var currenturl = getRelativeUrl(document.URL.replace(/\?.*?$/ig, '').toLowerCase()); /*写入菜单*/
