...
{
    public enum TopObjectType
    {
        Public = 0,
        Private = 1,
    }

    public class TopObject : ITopObject
    {
        ...
        public void Enter()
        {
            using (var repository = new BvOverallsReponsitory())
            {
                ...
                {
                    repository.Insert(new Layer.Data.Sqls.BvOveralls.TopObjects
                    {
                ...
                {
                    repository.Update<Layer.Data.Sqls.BvOveralls.TopObjects>(new
                    {
                        Value = this.Value,
                        UpdateDate = DateTime.Now
        ...
        public void Abandon()
        {
            ...
            using (var repository = new BvOverallsReponsitory())
            {
                repository.Delete<Layer.Data.Sqls.BvOveralls.TopObjects>(item => item.ID == this.ID);
            }
        ...
        public static void Inputs(string language, params TopObject[] tops)
        {
            new System.Threading.Thread(() =>
            {
                using (var repository = new BvOverallsReponsitory())
                {
                    ...
                    {
                        int count = repository.ReadTable<Layer.Data.Sqls.BvOveralls.TopObjects>().Count(item => item.Language == language && item.Name == top.Name);
                        if (count == 0)
                        {
        ...
        static public void Import(string lang, params TopObject[] tops)
        {
            using (var repository = new BvOverallsReponsitory())
            {
                foreach (var top in tops)
                {
                    var old = repository.ReadTable<Layer.Data.Sqls.BvOveralls.TopObjects>().FirstOrDefault(item => item.Language == lang && item.Name == top.Name);
                    if (old == null)
                    {
                        top.ID = Guid.NewGuid().ToString("N");
                        top.Language = lang;
                    ...
                    {
                        repository.Update<Layer.Data.Sqls.BvOveralls.TopObjects>(new { top.Value, UpdateDate = DateTime.Now }, item => item.ID == old.ID);
                    }
                }
            }
        }
        ...
        public static object Outputs()
        {
            ...
            foreach (var langGroup in new Views.TopObjectsView().GroupBy(lang => lang.Language))
            {
