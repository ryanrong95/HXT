<%@ Page Title="" Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="Execute.aspx.cs" Inherits="Yahv.Finance.WebApp.Payer.FundTransfer.Execute" %>
            ...
            $('#PaymentMethord').combobox({
                ...
                onSelect: function (data) {
                    if (data.value == <%=(int)PaymentMethord.BankAcceptanceBill %> 
                        || data.value == <%=(int)PaymentMethord.CommercialAcceptanceBill %>) {
                        $('#tr_mo').show();
                        ...
                        //背书转让（调入账户是否为承兑户）
                        if (model.Data.IsAcceptanceFromPayee) {
                            $('#tr_decors').show();
                            $('td[name="td_tx"]').hide();
