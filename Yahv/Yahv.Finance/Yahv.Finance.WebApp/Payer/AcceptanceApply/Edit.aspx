<%@ Page Title="Title" Language="C#" MasterPageFile="~/Uc/Works.Master" CodeBehind="Edit.aspx.cs" Inherits="Yahv.Finance.WebApp.Payer.AcceptanceApply.Edit" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="server">
    ...
    <script>
        $(function () {
            $('#Type').combobox({
                data: model.AcceptanceTypes,
                textField: 'text',
                valueField: 'value',
                required: true,
                onSelect: function (record) {
                    var data = model.PayeeAccounts;
                    $('td[name="td_inPrice"]').show();

                    //背书转让
                    if (record.value == '<%= (int)AcceptanceType.Endorsement%>') {
                        data = model.PayerAccounts;
                    }

                    //贴现时候，不显示调入金额
                    if (record.value == '<%= (int)AcceptanceType.Discount%>') {
                        $('td[name="td_inPrice"]').hide();
                    }
            ...
            $('#MoneyOrderID').combogrid({
                ...
                onChange: function (q) {
                    ...
                    if (q.indexOf('MoneyOrder') < 0)
                        doSearch(q, model.MoneyOrders, ['PayerAccountName', 'Name', 'Code', 'Price'], $(this));
                },
                onSelect: function () {
                    var row = $('#MoneyOrderID').combogrid('grid').datagrid('getSelected');
                    $('#PayerAccountID').combogrid('setValue', row.PayeeAccountID);
                    $('#PayerPrice').numberbox('setValue', row.Price);
                    ...
                    //贴现，调入金额不默认
                    if ($('#Type').combobox('getValue') == '<%= (int)AcceptanceType.Discount %>') {
                        $('#PayeePrice').numberbox('setValue', '');
                    } else {
                        $('#PayeePrice').numberbox('setValue', row.Price);
                    }
                }
            });
    ...
    <div style="padding-bottom: 5px;">
        ...
        <table class="liebiao">
            ...
            <tr>
                ...
                <td name="td_inPrice">调入金额</td>
                <td name="td_inPrice">
                    <input id="PayeePrice" name="PayeePrice" class="easyui-numberbox" style="width: 200px;" data-options="required: false,precision: 2,min: 0" />
                </td>
            </tr>
            <tr>
                <td>申请人</td>
