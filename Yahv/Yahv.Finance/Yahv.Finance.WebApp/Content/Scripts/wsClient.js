...
(function ($) {
    ...
    //接口地址
    var ajaxUrl = {
        getEnterpriseData: '/Finance/BasicInfo/Companies/Infos.aspx?action=Index_WsClient',//获取企业数据(前20条)
        getEnterprises: '/Finance/BasicInfo/Companies/Infos.aspx?action=Search_WsClient&name=',//通过企业名称搜索企业的数据
        getEnterpriseById: '/Finance/BasicInfo/Companies/Infos.aspx?action=Getbyid_WsClient&id='
    }
    ...
    //编写Enterprise插件
    $.fn.WsClient = function (opt, param) {
        //如果options为string，则是方法调用
        if (typeof opt == 'string') {
            var method = $.fn.WsClient.methods[opt];
            if (method) {
        ...
        (function () {
            ...
            if (s) {
                ...
                options = $.extend(true, {}, $.fn.WsClient.defaults, data_options, options);
            } else {
                options = $.extend(true, {}, $.fn.WsClient.defaults, options);
            }
        })();
        ...
        if (options.value && options.value != "") {
            $(sender).WsClient('setVal', options.value);
    ...
    //企业插件的默认配置
    $.fn.WsClient.defaults = $.extend({}, $.fn.combobox.defaults, {
        width: 220,
        target: null,
        value: null,
        valueField: 'ID',
