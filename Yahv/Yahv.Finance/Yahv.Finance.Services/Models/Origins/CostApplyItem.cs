...
{
    public class CostApplyItem : IUnique
    {
        #region 事件
        ...
        public void Enter()
        {
            ...
            {
                //添加
                if (!reponsitory.ReadTable<Layers.Data.Sqls.PvFinance.CostApplyItems>().Any(item => item.ID == this.ID))
                {
                    this.ID = this.ID ?? PKeySigner.Pick(PKeyType.CostApplyItem);
                    reponsitory.Insert(new Layers.Data.Sqls.PvFinance.CostApplyItems()
                    {
                        ID = this.ID,
