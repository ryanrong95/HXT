...
namespace Yahv.Finance.Services.Models.Origins
{
    ...
    /// 资金申请
    /// </summary>
    public class CostApply : IUnique
    {
        #region 事件
        ...
        /// 资金申请用途
        /// </summary>
        public FundPurpose CostPurpose { get; set; }
        ...
        /// 资金用途
        /// </summary>
        public string CostPurposeDes { get; set; }
        ...
        public void Enter()
        {
            ...
            {
                //添加
                if (!reponsitory.ReadTable<Layers.Data.Sqls.PvFinance.CostApplies>().Any(item => item.ID == this.ID))
                {
                    this.ID = this.ID ?? PKeySigner.Pick(Yahv.Finance.Services.PKeyType.CostApply);
                    reponsitory.Insert(new Layers.Data.Sqls.PvFinance.CostApplies()
                    {
                        ID = this.ID,
                        ...
                        CostPurpose = (int)this.CostPurpose,
                ...
                {
                    reponsitory.Update<Layers.Data.Sqls.PvFinance.CostApplies>(new
                    {
                        PayeeAccountID = this.PayeeAccountID,
                        PayerAccountID = this.PayerAccountID,
                        Type = (int)this.Type,
