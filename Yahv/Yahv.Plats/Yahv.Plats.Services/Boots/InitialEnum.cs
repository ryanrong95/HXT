...
namespace Yahv.Plats.Services.Boots
{
    public class InitialEnum
    {
        ...
        void Enter<T>()
        {
            using (var repository = new PveCrmReponsitory())
            {
                var type = typeof(T);
                var query = Enum.GetValues(type).Cast<Enum>();
                var baseType = Enum.GetUnderlyingType(type);

                var exsits = repository.ReadTable<Layers.Data.Sqls.PveCrm.EnumsDictionaries>().
                ...
                foreach (var item in query)
                {
                    ...
                    {
                        repository.Insert(new Layers.Data.Sqls.PveCrm.EnumsDictionaries
                        {
                            ID = id,
                            Enum = type.Name,
                            IsFixed = true,
