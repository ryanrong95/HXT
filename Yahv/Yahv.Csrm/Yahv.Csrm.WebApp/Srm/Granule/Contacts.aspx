<%@ Page Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="Contacts.aspx.cs" Inherits="Yahv.Csrm.WebApp.Srm.Granule.Contacts" %>
            ...
            window.grid = $("#dg").myDatagrid({
                ...
                onCheck: function (index, row) {
                    ...
                    $.messager.confirm('确认', '确认将联系人分配给' + model.Admin.RealName + '吗？', function (r) {
                        if (r) {
                            $.post('?action=Bind', { contactid: row.ID, adminid: model.Admin.ID, supplierid: model.Supplier.ID }, function (result) {
                ...
                onUncheck: function (index, row) {
                    ...
                    $.messager.confirm('确认', '确认取消分配该联系人给' + model.Admin.RealName + '吗？', function (r) {
                        if (r) {
                            $.post('?action=UnBind', { contactid: row.ID, adminid: model.Admin.ID, supplierid: model.Supplier.ID }, function (result) {
                                top.$.messager.alert('操作提示', result.data, 'info');
                            });
                        }
                    })
