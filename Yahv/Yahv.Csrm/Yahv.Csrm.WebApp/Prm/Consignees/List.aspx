<%@ Page Title="" Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="Yahv.Csrm.WebApp.Prm.Consignees.List" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="server">
    <script>
        $(function () {
            ...
            $("#btnDel").click(function () {
                ...
                var del = [];
                var arry = $.map(rows, function (item, index) {
                    if (item.Status == '<%=(int)ApprovalStatus.Deleted%>') {
                        del.push(item.ID);
                    }
                    else {
                        return item.ID;
    ...
    <script>
        ...
        function showEditPage(id) {
            $.myDialog({
                title: "编辑到货地址信息",
                url: 'Edit.aspx?id=' + id + '&clientid=' + model.Entity.ID, onClose: function () {
        ...
        function showApprovePage(id) {
            $.myWindow({
                title: "到货地址",
                url: '../../Crm/PendingConsignees/Edit.aspx?id=' + id,
                onClose: function () {
                    window.grid.myDatagrid('flush');
                },
                width: "70%",
        ...
        function showAddPage() {
            $.myDialog({
                title: "添加到货地址",
                url: 'Edit.aspx?clientid=' + model.Entity.ID,
                onClose: function () {
                    window.grid.myDatagrid('flush');
                },
                width: "70%",
