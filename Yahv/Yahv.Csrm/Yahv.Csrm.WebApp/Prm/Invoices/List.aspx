<%@ Page Title="" Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="Yahv.Csrm.WebApp.Prm.Invoices.List" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="server">
    <script>
        $(function () {
            ...
            $("#btnEnable").click(function () {
                ...
                if (nomal.length) {
                    top.$.messager.alert('操作失败', "请勿重复操作");
                }
                else {
                    $.post('?action=Enable', { items: arry.toString(), companyid: model.Entity.ID }, function () {
            ...
            $("#btnDel").click(function () {
                ...
                if (arry.length > 0) {
                    $.messager.confirm('确认', '您确认想要删除选中的发票吗？', function (r) {
                        if (r) {
                            $.post('?action=del', { items: arry.toString(), companyid: model.Entity.ID }, function () {
            ...
            $("#btnUnable").click(function () {
                ...
                if (unable.length) {
                    ...
                    $.post('?action=Unable', { items: arry.toString(), companyid: model.Entity.ID }, function () {
    ...
    <script>
        ...
        function showEditPage(id) {
            $.myDialog({
                title: '编辑发票信息',
                url: 'Edit.aspx?id=' + id + "&supplierid=" + model.Entity.ID, onClose: function () {
        ...
        function showAddPage() {
            $.myDialog({
                title: '添加发票',
                url: 'Edit.aspx?supplierid=' + model.Entity.ID, onClose: function () {
                    window.grid.myDatagrid('flush');
                },
                width: "50%",
                height: "80%",
        ...
        function showApprovePage(id) {
            $.myWindow({
                title: "审批发票信息",
                url: '../../Srm/PendingInvoices/Edit.aspx?id=' + id,
                onClose: function () {
                    window.grid.myDatagrid('flush');
                },
                width: "50%",
    ...
    <table id="dg" style="width: 100%">
        <thead>
            <tr>
                ...
                %>
                <th data-options="field: 'Type',width:80">发票类型</th>
                ...
                <th data-options="field: 'TaxperNumber',width:150">纳税人识别号</th>
                <th data-options="field: 'Address',width:150">收票地址</th>
