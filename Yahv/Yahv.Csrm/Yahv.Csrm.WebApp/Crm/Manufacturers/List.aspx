<%@ Page Title="" Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="Yahv.Csrm.WebApp.Crm.Manufacturers.List" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="server">
    ...
    <script>
        ...
        function btnformatter(value, rowData) {
            ...
            if (rowData.Agent) {
                arry.push('<a id="btnUpd" href="#" class="easyui-linkbutton" data-options="iconCls:\'icon-yg-cancel\'" onclick="Agent(\'' + rowData.ID + '\',false)">取消代理</a> ');
            }
            else {
                arry.push('<a id="btnUpd" href="#" class="easyui-linkbutton" data-options="iconCls:\'icon-yg-confirm\'" onclick="Agent(\'' + rowData.ID + '\',true)">设为代理</a> ');
            }
            arry.push('</span>');
            return arry.join('');
        }
        ...
        //设置代理或取消代理
        function Agent(id, isgent) {
            var msg = isgent ? "设置成功" : "取消成功"
            $.post('?action=agent', { id: id, IsAgent: isgent }, function (success) {
                if (success) {
                    top.$.timeouts.alert({
                        position: "CC",
                        msg: msg,
    ...
    <table id="dg" style="width: 100%">
        <thead>
            <tr>
                ...
                <th data-options="field:'IsAgent',width:80">是否代理</th>
