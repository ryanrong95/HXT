<%@ Page Title="" Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="Yahv.Csrm.WebApp.Crm.WsConsignors.List" %>
    ...
    <script>
        $(function () {
            ...
            $("#btnEnable").click(function () {
                ...
                if (rows == 0) {
                    top.$.messager.alert('操作失败', '请选择要启用的提货地址');
                    return false;
                }
                var errors = [];
                var nomal = [];
                ...
                if (errors.length) {
                    top.$.messager.alert('操作失败', "包含正在审核中的提货地址，启用失败");
                }
                else if (nomal.length) {
                    top.$.messager.alert('操作失败', "包含状态正常的提货地址，请勿重复操作");
                }
                else {
            ...
            $("#btnDel").click(function () {
                var rows = $('#dg').datagrid('getChecked');
                if (rows == 0) {
                    top.$.messager.alert('操作失败', '请选择要删除的提货地址');
                    return false;
                }
                ...
                if (errors.length) {
                    top.$.messager.alert('操作失败', "包含正在审核中的提货地址，不能删除");
                }
                else if (del.length) {
                    top.$.messager.alert('操作失败', "包含状态以删除的提货地址，请勿重复操作");
                }
                else {
                    $.messager.confirm('确认', '您确认想要删除选中的提货地址吗？', function (r) {
                        if (r) {
            ...
            $("#btnUnable").click(function () {
                var rows = $('#dg').datagrid('getChecked');
                if (rows == 0) {
                    top.$.messager.alert('操作失败', '请选择要停用的提货地址');
                    return false;
                }
                var errors = [];
                var unable = [];
                ...
                if (errors.length) {
                    top.$.messager.alert('操作失败', "包含正在审核中的提货地址，停用失败");
                }
                else if (unable.length) {
                    top.$.messager.alert('操作失败', "包含已经停用的提货地址，请勿重复操作");
                }
                else {
    ...
    <script>
        ...
        function deleteItem(id) {
            $.messager.confirm('确认', '您确认想要删除该提货地址吗？', function (r) {
                if (r) {
                    $.post('?action=del', { id: id, supplierid: model.SupplierID, clientid: model.ClientID }, function () {
                        //top.$.messager.alert('操作提示', '删除成功!', 'info', function () {
                        //    grid.myDatagrid('flush');
                        //});
                        top.$.timeouts.alert({
        ...
        function showEditPage(id) {
            $.myDialog({
                title: "编辑提货地址",
                url: 'Edit.aspx?id=' + id + '&clientid=' + model.ClientID + '&supplierid=' + model
                .SupplierID, onClose: function () {
