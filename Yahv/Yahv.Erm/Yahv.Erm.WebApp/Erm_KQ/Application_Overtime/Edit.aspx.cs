    ...
    public partial class Edit : ErpParticlePage
    {
            ...
            try
            {
                ...
                if (files.Count > 0)
                {
                    for (int i = 0; i < files.Count; i++)
                    {
                        ...
                        {
                            FileDirectory dic = new FileDirectory(file.FileName, FileType.OvertimeApplication);
        ...
        protected void Submit()
        {
            try
            {
                ...
                string OvertimeExchangeType = Request.Form["OvertimeExchangeType"];
                ...
                {
                    throw new Exception("工作日时间，不能申请加班!");
                }

                var applicationDate = Erp.Current.Erm.Applications
                    .Where(item => item.ApplicantID == Staff && item.ApplicationType == Services.ApplicationType.Overtime)
                    .Where(item => item.Context.Contains(Date.ToString("yyyy-MM-dd")))
                    .Where(item => item.ID != id);
                if (applicationDate.Count() > 0)
                {
                    throw new Exception("已经申请了加班，不能重复申请!");
                }
                #endregion

                Application application = Erp.Current.Erm.Applications[id] ?? new Application();
                application.Title = "加班申请";
                application.Context = new OverTimeContext()
                {
                    Date = Date,
                    ApproveID = Manager,
                    ...
                    Reason = Reason,
                    OvertimeExchangeType = (OvertimeExchangeType)Enum.Parse(typeof(OvertimeExchangeType), OvertimeExchangeType),
                }.Json();
                ...
                application.ApplicationType = Services.ApplicationType.Overtime;

                application.Enter();

                //如果是部门负责人或总经理则直接到行政审批
                if (staff.PostionCode == ((int)Services.Common.PostType.Manager).ToString()|| staff.PostionCode == ((int)Services.Common.PostType.President).ToString())
                {
