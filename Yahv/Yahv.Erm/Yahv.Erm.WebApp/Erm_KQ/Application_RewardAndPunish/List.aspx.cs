    ...
    public partial class List : ErpParticlePage
    {
        ...
        protected object data()
        {
            ...
            var applications = Erp.Current.Erm.ApplicationsRoll
                .Where(item => item.ApplicationType == Services.ApplicationType.RewardAndPunish)
                .Where(expression);
            ...
            var result = data.Select(item => new
            {
                ...
                StaffName = item.RewardAndPunishContext.StaffName,
                Department = item.RewardAndPunishContext.Department,
                PositionName = item.RewardAndPunishContext.PositionName,

                RewardOrPunish = item.RewardAndPunishContext.RewardOrPunish,
                RewardType = item.RewardAndPunishContext.RewardType,
                PunishType = item.RewardAndPunishContext.PunishType,
                RewardDec = item.RewardAndPunishContext.RewardDec,
                PunishDec = item.RewardAndPunishContext.PunishDec,
                Reason = item.RewardAndPunishContext.Reason,
        ...
        Expression<Func<Application, bool>> Predicate()
        {
            ...
            //查询参数
            var Applicant = Request.QueryString["Applicant"];
            ...
            {
                predicate = predicate.And(item => item.ApplicantID == Applicant);
            }
            ...
            {
                predicate = predicate.And(item => item.Context.Contains(Staff));
            }
