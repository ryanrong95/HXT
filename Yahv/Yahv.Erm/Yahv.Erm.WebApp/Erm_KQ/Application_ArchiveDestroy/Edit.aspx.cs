    ...
    public partial class Edit : ErpParticlePage
    {
        ...
        protected void Submit()
        {
            ...
            try
            {
                ...
                string SupervisorName = Request.Form["SupervisorName"];
                //文件信息
                ...
                //销毁档案内容
                var archives = Request.Form["archives"].Replace("&quot;", "'").Replace("amp;", "");
                var archiveList = archives.JsonTo<List<ArchiveContext>>();
                #endregion

                var Audit = Alls.Current.Staffs.SingleOrDefault(item => item.PostionCode == PostType.Manager.GetHashCode().ToString() &&
                item.DepartmentCode == DepartmentType.审计部.GetHashCode().ToString());
                if (Audit == null)
                {
                    throw new Exception("系统没有设置审计部负责人");
                }
                ...
                application.Title = "单证档案销毁申请";
                application.Context = new ArchiveDestroyContext()
                {
                    ApproveID = Manager,
                ...
                application.ApplicationType = Services.ApplicationType.ArchiveDestroy;

                application.Enter();

                //如果是审计部负责人，则直接到总经理
                if (staff.PostionCode == PostType.Manager.GetHashCode().ToString() && staff.DepartmentCode == DepartmentType.审计部.GetHashCode().ToString())
                {
