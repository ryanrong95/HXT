...
{
    public partial class ListInterview : ErpParticlePage
    {
        ...
        protected object data()
        {
            ...
            var staffs = Erp.Current.Erm.Logs_StaffApprovalAll.Where(item => item.ApprovalStep == StaffApprovalStep.Interview).Where(expression);
        ...
        protected void Pass()
        {
            try
            {
                ...
                if (staff != null)
                {
                    ...
                    log2.ApprovalStep = StaffApprovalStep.BackgroundInvestigate;
        ...
        /// 导出应聘登记表
        /// </summary>
        protected void ExportApplicant()
        {
            try
            {
                ...
                var fileName = FileType.EmploymentForm.GetDescription() + "-" + staff.Name + DateTime.Now.Ticks + ".xlsx";
                FileDirectory fileDir = new FileDirectory(fileName, FileType.EmploymentForm);
                fileDir.CreateDirectory();
                string filePath = fileDir.DownLoadRoot + fileName;
                string TempletePath = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Content\\Template\\files\\应聘人员登记表.xlsx");
                staff.ExportEmploymentForm(filePath, TempletePath);
        ...
        /// 导出面试情况评估表
        /// </summary>
        protected void ExportInterviewEvaluation()
        {
            try
            {
                string StaffID = Request.Form["ID"];
                ...
                var fileName = FileType.InterviewEvaluationForm.GetDescription() + "-" + staff.Name + DateTime.Now.Ticks + ".xlsx";
                FileDirectory fileDir = new FileDirectory(fileName, FileType.InterviewEvaluationForm);
                fileDir.CreateDirectory();
                //实际保存位置
                string filePath = fileDir.DownLoadRoot + fileName;
                //模板位置
                string TempletePath = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Content\\Template\\files\\面试情况评估表.xlsx");
                //导出文件
                staff.ExportInterviewEvaluation(filePath, TempletePath);
                var fileUrl = @"../../Files/Download/" + fileName;
                Response.Write((new { success = true, message = "导出成功", fileUrl }).Json());
            }
            catch (Exception ex)
