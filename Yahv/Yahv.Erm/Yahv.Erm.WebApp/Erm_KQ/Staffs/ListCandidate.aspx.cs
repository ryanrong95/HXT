...
{
    public partial class ListCandidate : ErpParticlePage
    {
        ...
        {
            this.Model.ApprovalStatus = ExtendsEnum.ToDictionary<StaffApprovalStep>()
        ...
        protected object data()
        {
            ...
            var staffs = Erp.Current.Erm.Logs_StaffApprovalAll.GetIQueryableEx().Where(expression);
            ...
            var data = staffs.OrderByDescending(item => item.CreateDate).ToArray().Select(item => new
            {
                ...
                ApprovalStep = item.ApprovalStep.GetDescription(),
                StatusDec = item.StatusDec,
        ...
        Expression<Func<Logs_StaffApproval, bool>> Predicate()
        {
            ...
            {
                var status = (StaffApprovalStep)Enum.Parse(typeof(StaffApprovalStep), ApprovalStatus);
                predicate = predicate.And(item => item.ApprovalStep == status);
            }
            return predicate;
        }
