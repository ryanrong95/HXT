    ...
    public partial class ListOfMy : ErpParticlePage
    {
        ...
        protected object data()
        {
            ...
            var applications = Erp.Current.Erm.ApplicationsRoll
                .Where(item => item.ApplicationType == Services.ApplicationType.InternalTraining)
                .Where(expression);
            ...
            var data = applications.OrderByDescending(item => item.CreateDate).ToArray().Select(item => new
            {
                ...
                DepartmentType = item.InternalTrainingContext.DepartmentName,
                Manager = item.InternalTrainingContext.ApproveName,
                TrainingTime = item.InternalTrainingContext.TrainingTime.ToString("yyyy-MM-dd HH:mm:ss"),
                TrainingLocation = item.InternalTrainingContext.TrainingLocation,
                Reason = item.InternalTrainingContext.Reason,
        ...
        protected void Delete()
        {
            ...
            {
                del.Abandon();
                Yahv.Oplogs.Oplog(Yahv.Erp.Current, Request.Url.ToString(), nameof(Yahv.Systematic.Erm), "培训申请",
                    $"删除", del.Json());
            }
        }    
