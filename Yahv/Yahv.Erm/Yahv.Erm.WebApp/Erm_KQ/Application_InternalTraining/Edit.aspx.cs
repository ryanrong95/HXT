    ...
    public partial class Edit : ErpParticlePage
    {
            ...
            try
            {
                ...
                if (files.Count > 0)
                {
                    for (int i = 0; i < files.Count; i++)
                    {
                        ...
                        {
                            FileDirectory dic = new FileDirectory(file.FileName, FileType.InternalTrainingApplication);
        ...
        protected void Submit()
        {
            ...
            try
            {
                ...
                string[] TrainingMethods = Request.Form["TrainingMethods"].Split(',');
                string TrainingTimeStr = Request.Form["TrainingTime"];
                DateTime TrainingTime;
                DateTime.TryParse(TrainingTimeStr, out TrainingTime);
                string TrainingLocation = Request.Form["TrainingLocation"];
                string Reason = Request.Form["Reason"];
                string[] Trainees = Request.Form["Trainees"].Split(',');
                string TrainingContent = Request.Form["TrainingContent"];
                //文件信息
                ...
                application.Title = "培训申请";
                application.Context = new InternalTrainingContext()
                {
                    ApproveID = Manager,
                ...
                application.ApplicationType = Services.ApplicationType.InternalTraining;

                application.Enter();

                //如果是总经理或部门负责人，则直接到行政部审核
                if (staff.PostionCode == PostType.President.GetHashCode().ToString() || staff.PostionCode == PostType.Manager.GetHashCode().ToString())
                {
