    ...
    public partial class List : ErpParticlePage
    {
        ...
        protected object data()
        {
            ...
            var applications = Erp.Current.Erm.ApplicationsRoll
                .Where(item => item.ApplicationType == Services.ApplicationType.Offtime)
                .Where(expression);
            if (!Erp.Current.IsSuper)
            {
                //判断是否是总经理,总经理能看到所有人的
                var staff = Alls.Current.Staffs[Erp.Current.StaffID];
                if (staff != null && staff.PostionCode != ((int)PostType.President).ToString())
                {
            ...
            var result = data.Select(item => new
            {
                ...
                DepartmentType = item.OffTimeContext.DepartmentName,
                Manager = item.OffTimeContext.ApproveName,
                Dates = item.OffTimeContext.Dates,
                Days = item.OffTimeContext.Days,
                Reason = item.OffTimeContext.ApplyReason,
                Type = item.OffTimeContext.LeaveType.GetDescription(),
                ...
                AdminStatus = item.Applicant.Status,
            });
        ...
        protected void Delete()
        {
            ...
            {
                del.Abandon();
                Yahv.Oplogs.Oplog(Yahv.Erp.Current, Request.Url.ToString(), nameof(Yahv.Systematic.Erm), "请假申请",
                    $"删除", del.Json());
            }
        }
        ...
        protected void BackToWork()
        {
            ...
            {
                string id = Request.Form["ID"];
                var apply = Erp.Current.Erm.Applications[id];
                //权限重新启动
                apply.Applicant.PermissionOpen();
                //工作承接退回
                if (apply.Applicant.RoleID == "Role00027" || apply.Applicant.RoleID == "Role00028")
                {
                    if (!string.IsNullOrEmpty(apply.OffTimeContext.SwapStaff))
                    {
                        var apisetting = new WlAdminApiSetting();
                        var apiurl = ConfigurationManager.AppSettings[apisetting.ApiName] + apisetting.HandOverReturn;
                        var result = Yahv.Utils.Http.ApiHelper.Current.JPost<JMessage>(apiurl, new
                        {
                            AdminLeave = apply.ApplicantID,
                            AdminWork = apply.OffTimeContext.SwapStaff,
                            ApplyID = apply.ID,
                        });
                    }
                }
