    ...
    public partial class ListOfMy : ErpParticlePage
    {
        ...
        protected object data()
        {
            ...
            var applications = Erp.Current.Erm.ApplicationsRoll
                .Where(item => item.ApplicationType == Services.ApplicationType.WorkCard)
                .Where(expression);
            ...
            var data = applications.OrderByDescending(item => item.CreateDate).ToArray().Select(item => new
            {
                ...
                DepartmentType = item.WorkCardContext.DepartmentName,
                Manager = item.WorkCardContext.ApproveName,
                Reason = item.WorkCardContext.Reason,
        ...
        protected void Delete()
        {
            ...
            {
                del.Abandon();
                Yahv.Oplogs.Oplog(Yahv.Erp.Current, Request.Url.ToString(), nameof(Yahv.Systematic.Erm), "工牌补办申请",
                    $"删除", del.Json());
            }
        }    
