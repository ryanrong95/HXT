...
{
    public partial class ApprovalStep4 : ErpParticlePage
    {
        ...
        protected void Submit()
        {
            try
            {
                ...
                application.CurrentVoteStep.Summary = "已领取";
                application.Approval(true);
                Response.Write((new { success = true, message = "领取成功" }).Json());
            }
            catch (Exception ex)
            {
                Response.Write((new { success = false, message = "领取失败：" + ex.Message }).Json());
            }
        }
