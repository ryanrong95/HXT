...
namespace Yahv.Erm.WebApp.Erm_KQ.Application_WorkCard
{
    public partial class ApprovalStep2 : ErpParticlePage
    {
        ...
        protected void LoadData()
        {
            ...
            if (!string.IsNullOrEmpty(ApplicationID))
            {
                ...
                this.Model.ApplicationData = new
                {
                    ...
                    Reason = application.WorkCardContext.Reason,
                    AcceptDate = application.WorkCardContext.AcceptDate,
                };
            }
        }
            ...
            try
            {
                ...
                if (files.Count > 0)
                {
                    for (int i = 0; i < files.Count; i++)
                    {
                        ...
                        {
                            FileDirectory dic = new FileDirectory(file.FileName, FileType.WorkCardApplication);
        ...
        protected void Submit()
        {
            try
            {
                ...
                var context = application.WorkCardContext;
                context.AcceptDate = DateTime.Now;
                ...
                Response.Write((new { success = true, message = "受理成功" }).Json());
            }
            catch (Exception ex)
            {
                Response.Write((new { success = false, message = "受理失败：" + ex.Message }).Json());
            }
        }
