<%@ Page Title="" Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="Edit.aspx.cs" Inherits="Yahv.Erm.WebApp.Erm_KQ.Application_ReSign.Edit" %>
        ...
        $(function () {
            ...
            $('#Date').datebox({
                ...
                onChange: function () {
                    ...
                    if (firstLoad) {
                        date = model.ApplicationData.Date;
                        firstLoad = false;
                    }
                    ajaxLoading();
                    $.post('?action=DateChange', { Name: name, Date: date }, function (res) {
                        ajaxLoadEnd();
                        var result = JSON.parse(res);
                        if (result.success) {
                            $('#AttendRecord').textbox('setValue', result.data.AttendStr);
                            $('#AmOn').checkbox({ checked: result.data.AmOn });
                            $('#AmOff').checkbox({ checked: result.data.AmOff });
                            $('#PmOn').checkbox({ checked: result.data.PmOn });
                            $('#PmOff').checkbox({ checked: result.data.PmOff });

                            $("#ReSignTimes").text(result.data.ReSignTimes);
                        }
                        else {
                            top.$.timeouts.alert({ position: "TC", msg: result.message, type: "error" });
                            $('#AttendRecord').textbox('setValue', "");
                            $('#AmOn').checkbox({ checked: false });
                            $('#AmOff').checkbox({ checked: false });
                            $('#PmOn').checkbox({ checked: false });
                            $('#PmOff').checkbox({ checked: false });
                            $("#ReSignTimes").text("0");
                        }
                    })
                }
            ...
            $("#btnSubmit").click(function () {
                ...
                data.append('AmOn', $('#AmOn').checkbox('options').checked);
                data.append('AmOff', $('#AmOff').checkbox('options').checked);
                data.append('PmOn', $('#PmOn').checkbox('options').checked);
                data.append('PmOff', $('#PmOff').checkbox('options').checked);
                data.append('ReSignTimes', $("#ReSignTimes").text());
                //文件信息
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphForm" runat="server" style="border: none">
    <div class="easyui-layout" style="width: 100%; height: 100%; border: none">
        <div data-options="region:'center'" style="border: none">
            <table id="tab1" class="liebiao">
                <tr>
                    <td class="lbl">补签员工：</td>
                    <td>
                ...
                <tr>
                    <td class="lbl">补签日期：</td>
                    <td>
                        <input id="Date" class="easyui-datebox" style="width: 350px;" />
                    </td>
                    <td class="lbl">打卡记录：</td>
                    <td>
                        <input id="AttendRecord" class="easyui-textbox" style="width: 350px;" data-options="disabled:true" />
                    </td>
                </tr>
                <tr>
                    <td class="lbl">补签时间：</td>
                    <td colspan="3">
                        <input id="AmOn" name="AmOn" class="easyui-checkbox" value="true"
                            data-options="label:'上午上班',labelPosition:'after',disabled:true">&nbsp&nbsp&nbsp&nbsp
                        <input id="AmOff" name="AmOff" class="easyui-checkbox" value="true"
                            data-options="label:'上午下班',labelPosition:'after',disabled:true">&nbsp&nbsp&nbsp&nbsp
                        <input id="PmOn" name="PmOn" class="easyui-checkbox" value="true"
                            data-options="label:'下午上班',labelPosition:'after',disabled:true">&nbsp&nbsp&nbsp&nbsp
                        <input id="PmOff" name="PmOff" class="easyui-checkbox" value="true"
                            data-options="label:'下午下班',labelPosition:'after',disabled:true">
                    </td>
                </tr>
                <tr>
        ...
        <div data-options="region:'south',height:40" style="background-color: #f5f5f5">
            <div style="float: left; margin-left: 5px; margin-top: 8px;">
                <span style="color: blue">本月已补签次数：</span><span id="ReSignTimes" style="color: red"></span><span style="color: blue">次</span>
            </div>
