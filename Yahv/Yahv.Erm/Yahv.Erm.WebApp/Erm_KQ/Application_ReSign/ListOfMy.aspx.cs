    ...
    public partial class ListOfMy : ErpParticlePage
    {
        ...
        protected object data()
        {
            ...
            var applications = Erp.Current.Erm.ApplicationsRoll
                .Where(item => item.ApplicationType == Services.ApplicationType.ReSign)
                .Where(expression);
            ...
            var data = applications.OrderByDescending(item => item.CreateDate).ToArray().Select(item => new
            {
                ...
                DepartmentType = item.ReSignContext.DepartmentName,
                Manager = item.ReSignContext.ApproveName,
                Date = item.ReSignContext.Date.ToString("yyyy-MM-dd"),
        ...
        protected void Delete()
        {
            ...
            {
                del.Abandon();
                Yahv.Oplogs.Oplog(Yahv.Erp.Current, Request.Url.ToString(), nameof(Yahv.Systematic.Erm), "补签申请",
                    $"删除", del.Json());
            }
        }
