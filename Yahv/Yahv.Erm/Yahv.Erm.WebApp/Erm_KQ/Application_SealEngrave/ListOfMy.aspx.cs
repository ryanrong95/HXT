    ...
    public partial class ListOfMy : ErpParticlePage
    {
        ...
        protected object data()
        {
            ...
            var applications = Erp.Current.Erm.ApplicationsRoll
                .Where(item => item.ApplicationType == Services.ApplicationType.SealEngrave)
                .Where(expression);
            ...
            var data = applications.OrderByDescending(item => item.CreateDate).ToArray().Select(item => new
            {
                ...
                DepartmentType = item.SealEngraveContext.DepartmentName,
                Manager = item.SealEngraveContext.ApproveName,
                SealName = item.SealEngraveContext.SealName,
                SealShape = item.SealEngraveContext.SealShape,
                SealSize = item.SealEngraveContext.SealSize,
                Reason = item.SealEngraveContext.Reason,
