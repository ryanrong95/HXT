...
{
    public partial class ApprovalStep2 : ErpParticlePage
    {
        ...
        protected void LoadComboBoxData()
        {
            ...
            //印章类型
            this.Model.SealType = Erp.Current.Erm.SealCertificates.AsEnumerable().Select(item => new
            {
                Value = item.Name,
                Text = item.Name,
            ...
            //印章借用类型
            this.Model.SealBorrowType = ExtendsEnum.ToDictionary<SealBorrowType>().Select(item => new { Value = item.Value, Text = item.Value });
            //印章盖章人
            var ids = Erp.Current.Erm.SealCertificates.Select(item => item.StaffID).ToArray();
            this.Model.SealerData = staffs.Where(item => ids.Contains(item.ID))
                .Select(item => new
                {
                    Value = item.Admin.ID,
                    Text = item.Name
        ...
        protected void LoadData()
        {
            ...
            if (!string.IsNullOrEmpty(ApplicationID))
            {
                ...
                this.Model.ApplicationData = new
                {
                    ...
                    SealType = application.SealBorrowContext.SealType,
                    SealBorrowType = application.SealBorrowContext.SealBorrowType,
                    BorrowDate = application.SealBorrowContext.BorrowDate,
                    ReturnDate = application.SealBorrowContext.ReturnDate,
                    Reason = application.SealBorrowContext.Reason,
                };
            }
        }
            ...
            try
            {
                ...
                if (files.Count > 0)
                {
                    for (int i = 0; i < files.Count; i++)
                    {
                        ...
                        {
                            FileDirectory dic = new FileDirectory(file.FileName, FileType.SealApplication);
        ...
        protected void Submit()
        {
            try
            {
                ...
                string Sealer = Request.Form["Sealer"];
                //文件信息
                ...
                var context = application.SealBorrowContext;
                context.Sealer = Sealer;
                application.Context = context.Json();

                application.Fileitems = fileList;
                application.Enter();

                var step = new Services.Views.Origins.ApplyVoteStepsAll().SingleOrDefault(item => item.ApplicationID == id && item.VoteStep.OrderIndex == 3);
                step.AdminID = Sealer;
                step.Enter();

                //审批通过
                application.CurrentVoteStep.Status = Services.ApprovalStatus.Agree;
                application.CurrentVoteStep.Summary = "已安排盖章";
        ...
        protected void Reject()
        {
            try
            {
                ...
                application.CurrentVoteStep.Summary = "退回";
