...
{
    public partial class ListSpecial2 : ErpParticlePage
    {
        ...
        protected object data()
        {
            ...
            var applications = Erp.Current.Erm.ApplicationsRoll
                .Where(item => item.ApplicationType == ApplicationType.SealBorrow)
                .Where(expression); 

            applications = applications.Where(item => item.ApplicantID == "Admin00523" ||
                    item.Steps.Any(t => t.AdminID == "Admin00523"));

            var total = applications.Count();
            var data = applications.OrderByDescending(item => item.CreateDate).ToArray()
                .Where(t => t.SealBorrowContext.SealType == "合同专用章3")
