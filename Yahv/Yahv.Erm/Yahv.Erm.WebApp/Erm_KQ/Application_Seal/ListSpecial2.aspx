<%@ Page Title="" Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="ListSpecial2.aspx.cs" Inherits="Yahv.Erm.WebApp.Erm_KQ.Application_Seal.ListSpecial2" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="server">
    ...
    <script>
        ...
        function Edit(id) {
            ...
            var url = location.pathname.replace(/ListSpecial2.aspx/ig, 'Edit.aspx') + '?ID=' + id + '&AdminID=' + model.CurrentAdmin.ID;
            $.myWindow({
                title: title,
                url: url,
                onClose: function () {
        ...
        //详情
        function Detail(id) {
            var url = location.pathname.replace(/ListSpecial2.aspx/ig, 'Details.aspx') + '?ID=' + id;
            $.myWindow({
                title: "申请详情",
                url: url,
                onClose: function () {
        ...
        function Approval(index) {
            ...
            var url = null;
            if (row.StepName == "总经理审批") {
                url = location.pathname.replace(/ListSpecial2.aspx/ig, 'ApprovalStep1.aspx') + '?ID=' + row.ID;
            }
            else if (row.StepName == "行政部安排盖章") {
                url = location.pathname.replace(/ListSpecial2.aspx/ig, 'ApprovalStep2.aspx') + '?ID=' + row.ID;
            }
            else {
                url = location.pathname.replace(/ListSpecial2.aspx/ig, 'ApprovalStep3.aspx') + '?ID=' + row.ID;
            }
            $.myWindow({
                title: row.StepName,
                url: url,
