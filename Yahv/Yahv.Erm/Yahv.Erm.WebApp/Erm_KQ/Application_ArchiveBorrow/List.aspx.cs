    ...
    public partial class List : ErpParticlePage
    {
        ...
        protected object data()
        {
            ...
            var applications = Erp.Current.Erm.ApplicationsRoll
                .Where(item => item.ApplicationType == Services.ApplicationType.ArchiveBorrow)
                .Where(expression);
            ...
            var result = data.OrderByDescending(item => item.CreateDate).ToArray().Select(item => new
            {
                ...
                DepartmentType = item.ArchiveBorrowContext.DepartmentName,
                Manager = item.ArchiveBorrowContext.ApproveName,
                BorrowDate = item.ArchiveBorrowContext.BorrowDate.ToString("yyyy-MM-dd"),
                ReturnDate = item.ArchiveBorrowContext.ReturnDate.ToString("yyyy-MM-dd"),
                Reason = item.ArchiveBorrowContext.Reason,
                ArchiveName = item.ArchiveBorrowContext.ArchiveName,
        ...
        Expression<Func<Application, bool>> Predicate()
        {
            ...
            var ArchiveName = Request.QueryString["ArchiveName"];
            ...
            {
                predicate = predicate.And(item => item.Context.Contains(ArchiveName));
            }
