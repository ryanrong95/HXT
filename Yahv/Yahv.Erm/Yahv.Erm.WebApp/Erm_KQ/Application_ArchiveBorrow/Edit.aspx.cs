...
namespace Yahv.Erm.WebApp.Erm_KQ.Application_ArchiveBorrow
{
    public partial class Edit : ErpParticlePage
    {
        ...
        protected void LoadComboBoxData()
        {
            ...
            var finance = staffs.FirstOrDefault(item => item.PostionCode == PostType.Manager.GetHashCode().ToString() && item.DepartmentCode == DepartmentType.财务部.GetHashCode().ToString());
            this.Model.FinanceAdmin = new
            {
                ID = finance?.Admin.ID,
            };
            //部门类型
        ...
        protected void LoadData()
        {
            ...
            if (!string.IsNullOrEmpty(ApplicationID))
            {
                ...
                this.Model.ApplicationData = new
                {
                    ...
                    BorrowDate = application.ArchiveBorrowContext.BorrowDate.ToString("yyyy-MM-dd HH:mm:ss"),
                    ReturnDate = application.ArchiveBorrowContext.ReturnDate.ToString("yyyy-MM-dd HH:mm:ss"),
                    Reason = application.ArchiveBorrowContext.Reason,
                    ArchiveName = application.ArchiveBorrowContext.ArchiveName,
                    Keeper = application.ArchiveBorrowContext.Keeper,
                    Financer = application.ArchiveBorrowContext.Financer,
                };
            }
        }
            ...
            try
            {
                ...
                if (files.Count > 0)
                {
                    for (int i = 0; i < files.Count; i++)
                    {
                        ...
                        {
                            FileDirectory dic = new FileDirectory(file.FileName, FileType.ArchiveBorrowApplication);
        ...
        protected void Submit()
        {
            ...
            try
            {
                ...
                string Financer = Request.Form["Financer"];
                ...
                application.Title = "单证档案借阅申请";
                application.Context = new ArchiveBorrowContext()
                {
                    ApproveID = Manager,
                ...
                application.ApplicationType = Services.ApplicationType.ArchiveBorrow;

                application.Enter();

                //如果是负责人或总经理则直接到行政部
                if (staff.PostionCode == PostType.Manager.GetHashCode().ToString() || staff.PostionCode == PostType.President.GetHashCode().ToString())
                {
