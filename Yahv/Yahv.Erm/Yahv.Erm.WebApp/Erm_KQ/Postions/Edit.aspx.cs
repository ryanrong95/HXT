...
namespace Yahv.Erm.WebApp.Erm_KQ.Postions
{
    public partial class Edit : ErpParticlePage
    {
        ...
        {
            var position = e.Object as Postion;
            BindingWages(position);

            //修改配置文件
            var info = Services.Common.XdtInfoHelper.GetConfig();
            if (!info.Positions.Contains(position.Name))
            {
                info.Positions.Add(position.Name);
                Services.Common.XdtInfoHelper.SetConfig(info);
            }
