...
<template>
  <div>
    <div class="top">
      ...
      <p class="itemlable">
        <label>付款公司：</label>
        <Select v-model="search.s_payer" filterable style="width: 73%">
          <Option
    ...
    <Drawer title="付款详情" :width="80"  v-model="showDetail" @on-visible-change="changevisible">
...
import { getclients,GetCompanies,GetCurrencies,showExpendlist} from "../../api/CgApi";
export default {
  name: "IncomeList",
  data() {
    return {
  ...
  console.log(lastDayOfCurMonth)
    ...
    show(row) {
      ...
      this.$router.push({
        path: "/Expendlist/ExpendlistDetail",
        query: {
          ...
          payer:row.Payer, //收款公司ID, 点击记录中的PayeeID
    ...
    searchdata(){
      this.loading=true
      showExpendlist(this.search).then(res=>{
        this.loading=false
