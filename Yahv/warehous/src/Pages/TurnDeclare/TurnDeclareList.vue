  ...
  <div>
    ...
    <div style>
      <p class="inputwidth" v-if="warehouseID.indexOf('SZ')==-1">
        ...
        <Input v-model.trim="search_data.Key" @on-enter='serch_btn' placeholder="运单号/入仓号/订单号/运输批次" style="width:215px" />
      </p>
      <p class="inputwidth" v-else>
        <label for="订单号/运单号">运单号/入仓号/订单号/ID：</label>
        <Input v-model.trim="search_data.Key" @on-enter='serch_btn' placeholder="运单号/入仓号/订单号/ID" style="width:215px" />
      </p>
      <p class="inputwidth">
...
import { cgCustomsStorageShow } from "../../api/CgApi";
...
export default {
  ...
  created() {
    if (this.$route.params.id == 10) {
      ...
      this.search_data.ExcuteStatus = this.testarrstate.join(",");
  ...
  beforeRouteUpdate(to,from,next){
        if(from.fullPath=='/TurnDeclare/10'&&to.fullPath=='/TurnDeclare/215'){
             this.reload(); //刷新当前页面  
        }else if(from.fullPath=='/TurnDeclare/215'&&to.fullPath=='/TurnDeclare/10'){
            this.reload(); //刷新当前页面 
        }else{
  ...
  watch:{
    ...
    console.log(to.path)
    var that=this
    if(to.path=='/TurnDeclare/10'||to.path=='/TurnDeclare/215'){
  ...
  methods: {
    ...
    showDrawer(id, type) {
      ...
      this.$router.push({ path: "/TurnDeclare/TurnDeclareDetail/" + id });
    },
    closDrawer() {
      this.$router.go(-1);
    ...
    pickingsout(data) {
      var _this = this;
      cgCustomsStorageShow(data).then(res => {
         this.loading = false;
