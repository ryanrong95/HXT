...
namespace WinApp.Services
{
    ...
    public class GeckoHelper
    {
        ...
        {
            //if (session.Data?.WsOrderID != null)
            //{
            //    session.Data.WsOrderID = session.Data.WsOrderID.Split(',').FirstOrDefault();
            //}
        ...
        {
            //for (var i = 0; i< session.PhotoMapes.Length; i++)
            //{
            //    var photoMap = session.PhotoMapes[i];
            //    if (photoMap.Data.WsOrderID != null)
            //    {
            //        photoMap.Data.WsOrderID = photoMap.Data.WsOrderID.Split(',').FirstOrDefault();
            //    }
            //}
        ...
        {
            //if (map.Data?.WsOrderID != null)
            //{
            //    map.Data.WsOrderID = map.Data.WsOrderID.Split(',').FirstOrDefault();
            //}
        ...
        static public void PrintDeliveryList(JObject data)
        {
            ...
            data["waybillinfo"]["AppointTime"] = DateTime.Now;
            form.Show();
        ...
        static public void ReprintFaceSheet(JObject data)
        {
            ...
            if (result.Source == (int)Yahv.PsWms.PvRoute.Services.PrintSource.SF)
            {
                ...
                ////采取顺丰云打印，Spire.Pdf静默打印形式
                //SFWaybillPrinter printer = new SFWaybillPrinter();
                ////打印
                //printer.CloudWayBillPrinter(result.SendJson,result.MainID, printerName, result.MyID);
                
                //采用顺丰云打印，避免在其他处打印找不到文件的问题，采取先下载再打印的方式
                PrinterHelper.SF_ClouldPrintHelper(code, "", result.MainID??"", false);                
