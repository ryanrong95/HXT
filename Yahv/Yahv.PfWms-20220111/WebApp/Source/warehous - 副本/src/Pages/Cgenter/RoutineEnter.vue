...
td /deep/.ivu-table-cell {
    padding-left: 5px;
    padding-right: 5px;
}
th /deep/.ivu-table-cell {
    padding-left: 5px;
    padding-right: 5px;
}
.sethover{
...
<template>
  <div>
    <div style="width:100%;">
        ...
        <div style="width:100%;min-height:200px;background:#f5f7f9;margin:15px 0">
          <Row >
            <Col style="width: 20%;float: left;">
              <ul class="detail_li detail1">
                ...
                <li class="itemli">
                  ...
                  <span>{{details.waybillinfo.ExcuteStatusDes}}</span>
                </li>
                <li class="itemli">
                ...
                <li class="itemli" v-if="details.waybillinfo.Conditions!=undefined">
                  <Icon type="md-alert" v-if="Conditionstype" style="font-size: 22px;color: #da2828;" />
                   <Tag color="geekblue" v-if="details.waybillinfo.Conditions.AgencyCheck==true">代检查</Tag>
                   <Tag color="geekblue" v-if="details.waybillinfo.Conditions.AgencyPayment==true">代垫货款</Tag>
                   <Tag color="geekblue" v-if="details.waybillinfo.Conditions.AgencyReceive==true">代收货款</Tag>
                   <Tag color="geekblue" v-if="details.waybillinfo.Conditions.ChangePackaging==true">代收货款</Tag>
                   <Tag color="geekblue" v-if="details.waybillinfo.Conditions.LableServices==true">LableServices</Tag>
                   <Tag color="geekblue" v-if="details.waybillinfo.Conditions.PayForFreight==true">垫付运费</Tag>
                   <Tag color="geekblue" v-if="details.waybillinfo.Conditions.Repackaging==true">重新包装</Tag>
                   <Tag color="geekblue" v-if="details.waybillinfo.Conditions.UnBoxed==true">拆箱验货</Tag>
                   <Tag color="geekblue" v-if="details.waybillinfo.Conditions.VacuumPackaging==true">真空包装</Tag>
                   <Tag color="geekblue" v-if="details.waybillinfo.Conditions.WaterproofPackaging==true">防水包装</Tag>
                </li>
              </ul>
            </Col>
            <Col style="width: 26%;float: left;">
              <ul class="detail_li">
                <li class="itemli">
                  <span class="detail_title2">操作时间：</span>
                ...
                <li class="itemli">
                  ...
                  <span>{{details.waybillinfo.TypeDes}}</span>
                  <!-- <a href="">历史到货
                         <Badge :count="10"></Badge>
                  </a>-->
                  <Badge :count="historydetail.waybillLIst.length" :offset="[9,-12]" class-name="demo-badge-alone" v-if="historydetail.waybillLIst.length>0">
            ...
            <Col style="width: 31%;float: left;">
              <ul class="detail_li">
                ...
                <li class="itemli">
                  ...
                  <span>
                     <Select v-model="details.waybillinfo.Carrier" style="width:60%">
                      <Option
                ...
                <li class="itemli">
                  ...
                  <span>
                    ...
                    <span v-if="details.waybillinfo.ConsignorPlace!=0">{{details.waybillinfo.ConsignorPlaceDes}}</span>
                    <span v-else style="color:red;">暂无输送地</span>
                    <Icon class="sethover" @click="clickClient(details.waybillinfo.ConsignorPlaceID,'ClientCode')" type="md-create" />
                  </span>
                </li>
              </ul>
            </Col>
            <Col style="width: 23%;float: left;">
              <ul class="detail_li" style="margin-left:20px;">
                <li class="itemli">
                  ...
                  <div class="setupload">
                    <Button type="primary" size="small" icon="ios-cloud-upload" @click="SeletUpload(details.waybillinfo.WaybillID)">传照</Button>
                  </div>
                  <div class="setupload">
                ...
                <li style="clear: both;">
                  <div v-for="(item,index) in details.waybillinfo.Files" v-if="index<10">
                    <span>{{item.CustomName}}</span>
                    <Icon type="ios-trash-outline" @click.native="handleRemove(item)"></Icon>
                  </div>
                </li>
                <li style="position: absolute; top: 147px; right: 0;" v-if="details.waybillinfo.WaybillType==1">
                  <Button type="primary" @click="TakeGoods">我去提货</Button>
                </li>
              </ul>
            </Col>
          </Row>
      ...
      <div class="itembox">
        ...
        <div style="margin:15px 0">
          ...
          <Button type="primary" @click="SetStorehouse">一键入库</Button>
        ...
        <!--   <Button type="primary" @click="imgprint">图片打印</Button> -->
          ...
          <div style="float:right" v-if="details.waybillinfo.OStatus!=200">
            <Button type="primary" shape="circle" icon="md-checkmark" @click="finish_btn">入库完成</Button>
            <Button
              type="warning"
              shape="circle"
          ...
          <div class="detail_tablebox">
            ...
            >
              ...
              <template slot-scope="{ row, index }" slot="Arrival">
                <span>{{row.SortedQuantity}}&nbsp;/&nbsp;{{row.Quantity}}</span>
                 <!-- <span>0&nbsp;/&nbsp;{{row.Quantity}}</span> -->
              </template>
              <template slot-scope="{ row, index }" slot="OriginDes">
                <span>{{row.originDes}}</span>
                <Icon class="sethover" @click="clickClient(row.originID,row)" type="md-create" />
                <!-- <Select 
              ...
              <template slot-scope="{ row, index }" slot="operation">
                ...
                <div class="setupload">
                  ...
                  <Button type="primary" icon="md-reverse-camera" @click="fromphotos(row.ID)">拍照</Button>
                </div>
                <div class="setupload">
                  <Button
                    v-if="row.iscx!=false"
            ...
            <div class="successbtn" v-if="details.waybillinfo.OStatus!=200">
              <Button type="primary" icon="md-checkmark" @click="finish_btn">入库完成</Button>
              <!-- <Button type="error">到货异常</Button> -->
              <Button type="warning" icon="ios-alert-outline" @click="isAbnormal=true">到货异常</Button>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- 异常到货 开始-->
      <Modal v-model="isAbnormal" title="到货异常" @on-ok="Abnormal_btn" @on-cancel="closeerror">
        <div slot="close">
          <Icon
            type="md-close-circle"
            color="rgb(33, 28, 28)"
    ...
    <!-- 收支明细 开始 -->
    <Modal v-model="Budgetdetail" width="55%"  :closable="false" :mask-closable="false" :footer-hide="true">
...
// import Vue from 'vue'
import { Noticedetail, sortingupload ,search_detail,getWayParter,Carriers,History,GetUsableShelves,TakeGoods} from "../../api";
import { CgDetail,cgenter} from "../../api/CgApi"; //引入api 的接口
  ...
  methods: {
    ...
    search_pro(){
        console.log(this.searchkey)
          ...
          search_detail(warehouseID,this.details.wayBillID,encodeURI(this.searchkey)).then(res => {
              console.log(res)
              if(res.obj!=""){
    ...
    setWarehousing(data){//确定入库，调取后台入库接口
      // sortingupload(data).then(res => {
      //   if (res.val == 0) {
      //     //成功之后调用父组件的关闭方法
      //     if (this.fatherMethod) {
      //       this.$Message.success("入库完成，一秒后自动关闭");
      //       var that = this;
      //       setTimeout(function() {
      //         that.fatherMethod();
      //       }, 1000);
      ...
      //   console.log(uploaddata)
        ...
        for(var i=0,len=deatilarr.length;i<len;i++){
          ...
          if(deatilarr[i].Quantity!=""||deatilarr[i].Quantity!=null){
            ...
            "Product": {
              ...
              "DateCode":deatilarr[i].Product.DateCode,
              "Origin": deatilarr[i].Product.Origin
            },
            "Files": [
              {
                "FileID": "sad",
        ...
        var obj={
          Waybill:{
              ...
              Code:this.details.waybillinfo.Code,
        ...
        this.setWarehousing(datas);
        // console.log(JSON.stringify(obj))
        this.WarehousingMsg=false;
    },
    cancel_Warehousing(){ //点击取消按钮，取消入库
        this.WarehousingMsg=false;
    },
    ...
    getdetail_data(id) { //初始化数据
      if (id != "") {
        ...
        Noticedetail(data).then(res => {
          ...
          // this.Summary=res.Summary;//备注说明异常原因
          console.log(res.obj.Data)
          // this.ClientCode=res.waybill.CorPlace
          var obj=res.obj.Notices;
          var objlength=obj.length
          this.loading=false;

          for ( i in res.obj.Conditions){
            console.log(res.obj.Conditions[i])
          ...
          for(var i=0;i<objlength;i++){  //为清单打印保留原始数据
            var item={
    ...
    changehouse(){
      ...
      for(var i=0;i<detaiitem.length;i++){
          for(var j=0;j<SelectRows.length;j++){
            if(detaiitem[i].ID==SelectRows[j].ID){
    ...
    historywaybill(name){  //显示历史到货详情数据
   ...
   changConveyingplace(value,row){  //改变库位号
      // console.log(value)
      // console.log(row)
   ...
   closeBudget(){  //收支明细关闭
    ...
    clickClient(value,type){  //显示更改输送地与原产地的方法
    ...
    primaryClientCode(){//确认更改地址
      ...
      if(this.chengevalue.type=='ClientCode'){
        ...
        for (var i = 0, l = newNotices.length; i < l; i++) {
              if(this.chengevalue.type.InputID==newNotices[i].InputID){
                  newNotices[i].originID=this.chengevalue.inputval.value;
                  newNotices[i].originDes=this.chengevalue.inputval.label;
                  break;
              }
          }
      }
    ...
    imgprint(){
        ...
        var data={
           setting:getsetting,
           data:[{
               src:"https://fixed.ic360.cn/newb1b/img/QR-Code.jpg"
             }]
            }
        FilePrint(data);
    },
