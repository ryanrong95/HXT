  ...
  data() {
    return {
      ...
      columns7: [
        ...
        //         props: {
        //         //将单元格的值给Input
        //         value: params.row.Place,
        //         },
        //         on: {
        //             "on-change"(event) {
        //                 //值改变时
        //                 //将渲染后的值重新赋值给单元格值
        //                 params.row.Place = event.target.value;
        //                 vm.Storages[params.index] = params.row;
        //             }
        //         }
        //     });
    ...
    submitbtn() {
      if (this.ShelveID == "") {
        ...
        if (
          ...
          this.Storages[0].Product.PartNumber == ""
        ) {
          Storagestrue = false;
        } else {
          Storagestrue = true;
          ...
          if (magtrue == true) {
            ...
            TempStorage(data).then(res => {
              ...
              if (res.val == 0) {
                ...
                this.$Message.error("暂存库录入失败，请检查录入信息是否正确");
              }
            });
          }
        }
