...
import { showIncomeDetail} from "../../api/CgApi";
export default {
  data() {
    return {
      loading: true,
  ...
  created(){
          ...
          this.payee=this.$route.query.payee, //收款公司ID, 点击记录中的PayeeID
  ...
  methods:{
    getdata(){
      var data={
          ...
          payee:this.payee, //收款公司ID, 点击记录中的PayeeID
          currency:this.currency, //币种
      }
      showIncomeDetail(data).then(res=>{
        this.loading=false
        this.dataDetail=res
      })
    }
