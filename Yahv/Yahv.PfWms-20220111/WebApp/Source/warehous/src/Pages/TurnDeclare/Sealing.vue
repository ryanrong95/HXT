...
<template>
  <div>
              ...
              >
               <template slot-scope="{ row, index }" slot="orderid">
                ...
                <template slot-scope="{ row, index }" slot="PartNumber">
                   <ul class="subCol">
                    <li v-for="item in row.Groups">{{item.Product.PartNumber}}
                ...
                <template slot-scope="{ row, index }" slot="CarrierName">
                  <ul class="subCol">
                    <li v-for="item in row.Groups">{{item.CarrierName}}</li>
                  </ul>
                </template>
                <template slot-scope="{ row, index }" slot="code">
                  <ul class="subCol">
                    <li v-for="item in row.Groups">{{item.wbCode}}</li>
                  </ul>
                </template>
    ...
    <!-- 修改数量 开始 -->
     <Modal
       ...
       >
        <div>
          <label for="">数量:</label>
          <Input v-model="NewQuantity" placeholder="请输入数量" />
        </div>
        <div slot="footer">
           <Button>取消</Button>
    ...
    <Modal
        ...
        <p v-for="item in overweight">{{item.TinyOrderID}} 
          <span style="padding-right:5px">总重量：{{item.TotalWeight}}</span>
          <span style="padding-right:5px">平均重量：{{item.TotalAvgWeight}}</span></p>
    </Modal>
  </div>
</template>
<script>
import { TinyOrderIDReportGroup, DeletePicking,BoxcodeEnter,CgBoxesShow,BoxcodeDelete,ModifyQuantity,cgCustomsStorageCloseBoxes} from "../../api/CgApi"; //引入api 的接口
export default {
  name: "Sealing",
  props: ["OrderID", "Type",'EnterCode','hietorgetDetail','WaybillID','ExcuteStatus'],
  data() {
    return {
      options3: {
          disabledDate (date) {
  ...
  mounted() {
    this.getlistdata()
    console.log(this.getExcuteStatus)
    if (this.Type == 1) {
  ...
  methods: {
    ...
    getlistdata(){
      TinyOrderIDReportGroup(this.OrderID).then(res=>{
        this.loading=false
        this.Normalinfo = res;
    ...
    ok_change(){
      if(this.newboxcode!=null&&this.newboxcode!=''){
        if(this.isclickbtn==true&&this.newboxcodeback!=null){
          ...
          source:60, /// ---->新增的, 就是当前通知类型,比如代报关 30, 转报关 60
          }
          BoxcodeEnter(data).then(res=>{
              if(res.success==false){
                this.$Message.error(res.data)
      ...
      if(this.overweight.length>0){
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
              ...
              this.cgCustomsStorageCloseBoxes(data)
          },
          onCancel: () => {
    ...
    cancel_sealing(){
     ...
     // 确认封箱
    cgCustomsStorageCloseBoxes(data){
      cgCustomsStorageCloseBoxes(data).then(res=>{
        if(res.success==true){
          this.loading=true
          this.getlistdata()
