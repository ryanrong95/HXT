...
import { EntryNoticeStatus,UnSortingData,HKDeliveryType } from "../../api/XdtApi"; //引入XDT api 的接口
...
export default {
  ...
  computed: {
    ...
    showdetail() {
      if (this.$store.state.common.showdetailBOX == true) {
        this.clear_time();
        this.timenmae = null;           
      } else {
        var _this = this;
      }
      return this.$store.state.common.showdetailBOX;
    },
    showtype() {
  ...
  methods: {
    ...
    showDrawer(id,orderID) {
      ...
      //     }
      //   });
        this.$store.dispatch("setshowdetailBOX",true);       
        this.$router.push({ name: 'HKDeclare', params: { entryNoticeID: id, orderID:orderID,mainOrderID:orderID.slice(0,16)} });
    },

    closDrawer() {
      this.$router.go(-1); //控制路由跳回原来页面
      var _this = this;
      this.$store.dispatch("setshowdetailBOX", false);     
    },
    changepage(value) {
      this.loading = true;
