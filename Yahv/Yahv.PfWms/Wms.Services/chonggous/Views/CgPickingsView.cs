    ...
    public class CgPickingsView : QueryView<object, PvWmsRepository>
    {
        ...
        public object GetPagelistData(int? pageIndex = 1, int? pageSize = 20)
        {
            ...
            var lotNumberView = from lotNumber in this.Reponsitory.ReadTable<Layers.Data.Sqls.PvWms.PvWmsOrderVoyageTopView>()
                                where ordersId.Contains(lotNumber.OrderID)
                                select new
                                {
                                    lotNumber.LotNumber,
                                    lotNumber.OrderID,
            ...
            var result = from waybill in waybills
                         ...
                         select new
                         {
                             ...
                             LotNumber = waybill.Source == CgNoticeSource.AgentBreakCustomsForIns ? waybill.LotNumber : string.Join(",", lotNumbers.Select(item => item.LotNumber).ToArray()),
        ...
        public object GetDetail(string WaybillID)
        {
            ...
            */
            ...
            var lotNumberView = from lotNumber in this.Reponsitory.ReadTable<Layers.Data.Sqls.PvWms.PvWmsOrderVoyageTopView>()
                                where lotNumber.OrderID == waybill.OrderID
        ...
        public CgPickingsView SearchByID(string id)
        {
            ...
            var linq = from waybill in waybillView
                       where waybill.ID.Contains(id) || waybill.Code.Contains(id) || waybill.OrderID.Contains(id) || (waybill.LotNumber.Contains(id))
        ...
        public void PartialPicking(JToken token)
        {
            ...
            if (waybilltop.NoticeType != (int)CgNoticeType.Boxing)
            {
                ...
                var tPickings = from item in pickings.Select(item => new
                {
                    item.StorageID,
                    item.Quantity,
                }).ToArray()
            ...
            if (Waybill["WarehouseID"].Value<string>().Contains("SZ"))
            {
                ...
                var result = Yahv.Utils.Http.ApiHelper.Current.JPost(Wms.Services.FromType.XdtSZShiped.GetDescription(), new SZToXDT { AdminID = adminid, WaybillID = Waybill["ID"].Value<string>() });
