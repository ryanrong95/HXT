  ...
  <div class="Customswindow" id="Customsbox">
    ...
    <div>
      <div class="tabletitle">
        ...
        <!-- <div style="text-align:center;float: left;" id="span1">#</div>
        <Row>
        <Col span="6" class="titleboxs">
            <span style="text-align:center" id="span2">订单号</span>
            <span style="text-align:center" id="span3">申报状态</span>
            <span style="text-align:center" id="span4">装箱时间</span>
            <span style="text-align:center" id="span5">装箱人</span>
        ...
        <Col span="4" class="titleboxs">
            <span style="text-align:center" id="span7">箱号</span>
            <span style="text-align:center" id="span8">库位</span>
            <span style="text-align:center" id="span9">规格</span>
            <span style="text-align:center" id="span10">总毛重</span>
        </Col>
        <Col span="13" class="titleboxs">
          <span style="text-align:center" id="span11">毛重</span>
          <span style="text-align:center" id="span12">型号</span>
          <span style="text-align:center" id="span13">品牌</span>
          <span style="text-align:center" id="span14">批次</span>        
               ...
               <template slot-scope="{ row, index }" slot="test1">
                  <Table :columns="columns3" :data="row.productdata" :show-header='false' class="producttable">
                    ...
                    <template slot-scope="{ row, index }" slot="Proaction">
                      <Button type="primary">变更箱号</Button>
                    </template>
                  </Table>
              </template>
            </Table>
    ...
    <div>
      <!-- <Table 
        ...
        <template slot-scope="{ row, index }" slot="ClientID">
          <ul class="arritem" v-for="item in row.Notices">
             <li class="itemPartNumber">{{item.Input.ClientName}}</li>
          </ul>
        </template>
        <template slot-scope="{ row, index }" slot="PartNumber">
          <ul class="arritem" v-for="item in row.Notices" v-if="item.Product!=null">
            <li>
        ...
        <template slot-scope="{ row, index }" slot="Datacode">
           <ul class="arritem" v-for="item in row.Notices" v-if="item.Product!=null">
             <li class="itemPartNumber">{{item.Input.DateCode}}</li>
           </ul>
        </template>
        <template slot-scope="{ row, index }" slot="Quantity">
          <ul class="arritem" v-for="item in row.Notices" v-if="item.Product!=null">
             <li class="itemPartNumber">{{item.Quantity}}</li>
           </ul>
        </template>
        <template slot-scope="{ row, index }" slot="place">
          <ul class="arritem" v-for="item in row.Notices" v-if="item.Product!=null">
             <li class="itemPartNumber">{{item.OriginDescription}}</li>
           </ul>
        </template>
        <template slot-scope="{ row, index }" slot="action">
          <ul v-for="item in row.Notices" class="arritem">
            <li>
...
export default {
  ...
  data() {
    ...
    columns2: [
              ...
              //                 console.log(vm.new_datas[params.index].boxdata[params.index])

              //                 vm.new_datas[params.index].boxdata[params.index] = params.row
              //               }
              //             }
              //           },
              //           vm.ShelveArr.map(function(type) {
