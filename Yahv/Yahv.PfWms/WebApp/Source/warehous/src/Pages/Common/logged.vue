    ...
    <div v-if="loglist.length>0">
      <ul class="loglistbox">
        <li v-for="(item,index) in loglist"  @mouseenter="clickcategory(index)">
          ...
          <span :class="{contentactive:categoryIndex==index}" class="content">{{item.Content}}</span>
...
<script>
import {cgLogOperators} from '../../api/CgApi'
export default {
  name: "logged",
  props: ["WaybillID"],
  data() {
 ...
 methods: {
     getlog() {
      cgLogOperators(this.search).then(res=>{
          console.log(res)
