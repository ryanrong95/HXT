...
<template>
  <div>
          ...
          <Row v-if="waybillinfo!=null">
            <Col style="width: 23%;float: left;">
              <ul class="detail_li detail1">
                ...
                <li class="itemli">
                  ...
                  <span>{{waybillinfo.ExcuteStatusName}}
                ...
                <li class="itemli">
                  ...
                  <span
                    style="color:'#ccc';font-width:600"
                  >{{waybillinfo.chgTotalCurrency}} {{waybillinfo.chgTotalPrice}}</span>
                </li>
                <li class="itemli">
              ...
              <ul class="detail_li">
                ...
                <li class="itemli">
                  ...
                  <span style="display: inline-block; width:70%; line-height: 1;">
                    <p v-for="(item,index) in waybillinfo.Supplier">
                      {{item.Supplier}}&nbsp;&nbsp;
                      <Tag color="volcano">LV {{item.SupplierGrade}}</Tag>
                    </p>
                  </span>
                </li>
                <li class="itemli">
                  ...
                  <span
                    v-if="warehouseID.indexOf('SZ')!=-1||(waybillinfo.Type==1||waybillinfo.Type==2) "
                  >{{waybillinfo.TypeName}}</span>
                  <Select
                    v-else
              ...
              <ul class="detail_li">
                ...
                <li class="itemli" v-if="waybillinfo.Type==2">
                  ...
                  <span>
                    ...
                    <Input
                      v-model="waybillinfo.CarNumberID"
                      :disabled="true"
                      style="width:60%"
                      placeholder="请输入车牌号"
                    />
            ...
            <Col style="width: 20%;float: left;">
              <ul class="detail_li" style="margin-left:20px;">
                <li class="itemli">运输批次号：{{waybillinfo.LotNumber}}</li>
                <li class="itemli">
                  <span
                    style="float:left;line-height: 27px;"
                    v-if="waybillinfo.ExcuteStatus!=215"
                  >发货情况拍照：</span>
      ...
      <div class="itembox">
        ...
        <div style="margin:15px 0">
          <ButtonGroup style="width:28%">
            ...
            />
            <Button style="float:left" @click="search_pro" type="primary" disabled>筛选</Button>
          ...
          <!-- <Button type="primary" @click="testprintbtn">打印测试</Button> -->
    ...
    <!-- 一键入箱 开始 -->
    <Modal v-model="showchangebox" title="选择箱号" @on-ok="ok_changebox" @on-cancel="cancel">
    ...
    <!-- 异常到货 结束-->
    <!-- 一键打印弹出 开始 -->
    <Modal v-model="showprint" title="打印">
      ...
      <CheckboxGroup v-model="fruit">
        ...
        <Checkbox label="送货单统计打印"></Checkbox>
...
export default {
    ...
    return {
      ...
      columns1: [
        ...
        },
        {
          ...
          render: (h, params) => {
            var vm = this;
            return h("span", {}, params.row.CurrentQuantity);
          }
        },
        {
          title: "箱号",
          ...
          render: (h, params) => {
            var vm = this;
            return h("span", {}, this.setboxsplit(params.row.BoxCode));
          }
        },
        {
          title: "毛重(kg)",
          ...
          render: (h, params) => {
            var vm = this;
            return h("Input", {}, params.row.Weight);
          }
        },
        {
          title: "图片",
      ...
      tableColumn: [
        { type: "seq", width: 50 },
        {
          ...
          slots: {
            default: ({ row, column }) => {
              return [<span>{row.TinyOrderID}</span>];
            }
          }
        },
        {
          ...
          slots: {
            default: ({ row, column }) => {
              return [<span>{this.waybillinfo.EnterCode}</span>];
            }
          }
        },
        {
          ...
          slots: {
            default: ({ row }, h) => {
              return [<span>{this.waybillinfo.ClientName}</span>];
            }
          }
        },
        {
          ...
          slots: {
            default: ({ row }, h) => {
              return [
                <span>{this.showDate2(this.waybillinfo.BoxingDate)}</span>
              ];
            }
          }
        },
  ...
  methods: {
    ...
    getdetail_data() {
      //获取详情页数据
      GetSzDetail(this.wayBillID).then(res => {
        ...
        if (this.warehouseID.indexOf("SZ") != -1) {
          ...
          if (res.Waybill.Type != 1 || res.Waybill.Type != 2) {
            //如果送货方式不是自提或送货上门，并且快放类型与快递方式为顺丰或跨域则显示运单打印
            if (res.Waybill.Extype != null && res.Waybill.ExPayType != null) {
              //处理打印的类型
              this.fruit = ["送货单打印", "运单打印"]; //打印选中
              this.Iswaybillprint = true;
            } else {
              this.fruit = ["送货单打印", "送货单统计打印"]; //打印选中
    ...
    finish_btn() {
      ...
      //     if(this.detailitem[i].BoxCode!=null){
      //       var obj={
      //           NoticeID: this.detailitem[i].ID,
      //           OutputID:this.detailitem[i].OutputID,
      //           BoxCode: this.detailitem[i].BoxCode,
      //           Quantity: this.detailitem[i].CurrentQuantity,
      //           Weight: this.detailitem[i].Weight,
      //           NetWeight: this.detailitem[i].NetWeight,
      //           Volume: this.detailitem[i].Pickings.Volume,
      //           Files: this.detailitem[i].Imagefiles
      //         }
    ...
    Removebackfun(file, type) {
      ...
      console.log(type);
      if (type == "Clientimg") {
        this.waybillinfo.SendGoodsFile.splice(
          this.waybillinfo.SendGoodsFile.indexOf(file),
          1
        );
      } else if (type == "Waybill") {
        this.waybillinfo.FeliverGoodFile.splice(
          this.waybillinfo.FeliverGoodFile.indexOf(file),
          1
        );
      } else {
    ...
    CgAllprint() {
      ...
      if (this.fruit.indexOf("送货单统计打印") != -1) {
        this.Deliverycount();
      }
    },
    //出库单打印功能
    ...
    Boxing_print() {
      ...
      if (this.warehouseID.indexOf("SZ") != -1) {
        ...
        //香港库房
        console.log("香港库房");
        if (this.waybillinfo.Type == 4) {
          //国际
          this.language = "EN";
          Numcopies = 2;
        } else if (this.waybillinfo.Type == 3) {
          //本港
          this.language = "TC";
          Numcopies = 2;
        } else {
      ...
      var printdata = [];
      GetSzPrintData(this.waybillinfo.ID).then(res => {
        printdata = res.sort(function(a, b) {
          return a.TinyOrderID.localeCompare(b.TinyOrderID);
        });
        var newarr = [];
        var _this = this;
        setTimeout(function() {
          for (var j = 0; j < 2; j++) {
            for (var i = 0; i < printdata.length; i++) {
              newarr.push(printdata[i]);
              //  console.log(printdata[i]);
            }
            //  console.log('--------'+newarr.length .toString());
          }
            ...
            setTimeout(
              function(param1) {
                var data = {
                  ...
                  waybillinfo: _this.waybillinfo,
                  listdata: param1,
                  Numcopies: 1
                };

                PrintSZDeliveryList(data);
                // console.log(data)
              },
              5 * counter++, item );
          }
    ...
    Deliverycount() {
      ...
      PrintDeliveryHomeList(data);
      // console.log(JSON.stringify(this.detailitem) )
    },
    //运单打印
    waybillprint() {
      var printdata = {
        ShipperCode: this.waybillinfo.Extype, //this.modelstype, //快递公司
        ExpType: this.waybillinfo.ExPayType, //this.IGetExpType, //货运类型
        Sender: {
          //发件人
