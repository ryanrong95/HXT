    ...
    <div style="width:100%;">
            ...
            <Col style="width: 30%;float: left;">
              <ul class="detail_li">
                ...
                <li class="itemli" v-if="waybillinfo.Type==2">
                  ...
                  <span>
                     <Input v-model="waybillinfo.CarNumberID" :disabled='true' style="width:60%" />
                  </span>
                </li>
                ...
                <li class="itemli" v-if="waybillinfo.Type!=2">
                  <span class="detail_title3">随&nbsp;&nbsp;货&nbsp;&nbsp;文&nbsp;&nbsp;件：</span>
                    <div v-for="(item,index) in waybillinfo.Files" v-if="item.Type==25" style="display: inline-block;">
                      <span class="linkurlcolor Filesbox" @click="clackFilesProcess(item.Url)">{{item.CustomName}}</span>
                      <a @click="fileprint(item.Url)">打印</a>
                    </div>
                </li>
      ...
      <div class="itembox">
        ...
        <div style="margin:15px 0">
          ...
          <!-- <Button type="primary" @click="PrintBoxcode" :disabled='waybillinfo.ExcuteStatus==215?true:false'>箱签打印</Button> -->
        ...
        <div>
          ...
          <div style="padding:20px">
             <!-- <Sealing ref="History" v-if="reFresh" :OrderID="waybillinfo.OrderID" Type="2" :EnterCode='waybillinfo.EnterCode' :uploadeCgDetail_new='uploadeCgDetail_new'></Sealing> -->
          </div>
        </div>
      </div>
    </div>
...
import {CgPickingsDetail,CgPickingsSearch,CgBoxesShow,CgpickingsErrorBtn,CgpickingsTurnDeclare,CgDeleteFiles,CgCarriers,BoxcodeEnter,CgEnterSeries,Getclientdata,BoxcodeDelete} from '../../api/CgApi'
...
export default {
  ...
  finish_btn() {
    if(this.SelectRow.length<=0){
      ...
      //           BoxCode: this.SelectRow[i].BoxCode,
      ...
      //     if(istrue==1&&index==this.SelectRow.length-1){
      //       this.WarehousingMsg = true 
      //     }
      //   }
    }
     ...
     setWarehousing(data) {
      //确定出库，调取后台出库接口
       CgpickingsTurnDeclare(data).then(res=>{
          console.log(res)
          if(res.Success==true){
    ...
    ok_Warehousing() {
      ...
      //  for(var i=0;i<this.pusharr.length;i++){
      //    this.pusharr[i].BoxCode=this.newboxcode
      //  }
        this.pusharr=[]
       for(var i=0,len=this.SelectRow.length;i<len;i++){
          var index=i
    ...
    TestAVGWeightsum(){
      if(this.sharevalue!=null){
        ...
        if(reg.test(this.sharevalue) == false||this.sharevalue==0){
          ...
          if(this.sharevalue<(this.AVGWeightsum*(1/2))||this.sharevalue>(this.AVGWeightsum*(3/2))){
