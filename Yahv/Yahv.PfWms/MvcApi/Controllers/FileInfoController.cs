...
namespace MvcApp.Controllers
{
    public class FileInfoController : Controller
    {
        ...
        public ActionResult Index(string CustomName = null, FileTypes Type = 0, FileTypesStatus Status=0)
        {
            try
            {
                Expression<Func<Wms.Services.Models.FileInfo, bool>> exp = item => true;
                if (!string.IsNullOrWhiteSpace(CustomName))
                {
                    exp = PredicateExtends.And(exp, item => item.CustomName.Contains(CustomName));
                }
                if (Type > 0)
                {
                    ...
                    exp = PredicateExtends.And(exp, item => item.Type.Equals(Type));
                }
                if (Status>0)
                {
                    ...
                    exp = PredicateExtends.And(exp, item => item.Status.Equals(Status));
                }
        ...
        [HttpPost]
        public ActionResult Index(Wms.Services.Models.FileInfo datas)
        {
            try
            {
                //保证数据的一致性
        ...
        private void Datas_EnterError(object sender, Yahv.Usually.ErrorEventArgs e)
        {
