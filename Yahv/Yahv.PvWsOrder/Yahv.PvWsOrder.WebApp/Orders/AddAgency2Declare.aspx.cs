...
{
    public partial class AddAgency2Declare : ErpParticlePage
    {
        ...
        private void LoadComboboxData()
        {
            ...
            this.Model.originData = ExtendsEnum.ToArray<Origin>()
                .Select(item => new { Value = item.ToString(), Text = item + " " + item.GetDescription() });
            //包装类型数据
        ...
        private void LoadData()
        {
            ...
            {
                ClientID = Erp.Current.WsOrder.WsClients.Single(t => t.Name == ClientName).ID;
            }
        ...
        protected object data()
        {
            ...
            var financeAccounts = new Yahv.PvWsOrder.Services.ClientViews.StorageViewForAgency2Declare().Where(t => stroageIDs.Contains(t.ID)).AsQueryable();

            return this.Paging(financeAccounts, item => new
            {
                StorageID = item.ID,
                InputID = item.InputID,
                DateCode = item.DateCode,
                CustomName = item.Product.CustomsName,
                ...
                Qty = item.Quantity,
                Unit = "007",
                TotalPrice = item.Input.UnitPrice * item.Quantity,
        ...
        protected void ImportProductInfo()
        {
            try
            {
                ...
                var products = Services.Common.Helper.ProductImport2(file);
        ...
        protected void SubmitOrder()
        {
            try
            {
                ...
                if (supplier != null)
                {
                    ...
                    Consignor.Place = sourceDistrict;
                ...
                {
                    Yahv.Services.Models.WayCharge WayCharge = new Yahv.Services.Models.WayCharge();
                ...
                wayCondition.IsTurnDeclare = true;
                ...
                string OutClientID = Request.Form["OutClientID"];
                string OutEnterCode = Request.Form["OutEnterCode"];
                var outClient = Erp.Current.WsOrder.WsClients.Single(t => t.ID == OutClientID);
                var outInvoiceID = outClient.StorageType != WsIdentity.Personal ?
                    Erp.Current.WsOrder.Invoices.Where(item => item.EnterpriseID == OutClientID).FirstOrDefault()?.ID :
                    Erp.Current.WsOrder.vInvoices.Where(t => t.EnterpriseID == OutClientID && t.IsDefault).FirstOrDefault()?.ID;
                if (string.IsNullOrEmpty(outInvoiceID))
                {
                    throw new Exception("客户的开票信息为Null");
                }

                Order orderHK = new Order();
                orderHK.ClientID = OutClientID;
                orderHK.Type = OrderType.Delivery;
                orderHK.InvoiceID = outInvoiceID;
                orderHK.PayeeID = "DBAEAB43B47EB4299DD1D62F764E6B6A";
                orderHK.Summary = summary;
                orderHK.CreatorID = orderHK.OperatorID = Erp.Current.ID;
                orderHK.EnterCode = OutEnterCode;
                orderHK.SettlementCurrency = Currency.HKD;

                orderHK.OrderOutput = order.OrderOutput;
                orderHK.OrderOutput.Waybill.ID = string.Empty;
                orderHK.OrderOutput.Waybill.Type = WaybillType.DeliveryToWarehouse;
                orderHK.OrderOutput.Waybill.Consignee = order.OrderOutput.Waybill.Consignor;
                orderHK.OrderOutput.Waybill.Summary = order.ID;
                orderHK.OrderOutput.Waybill.Source = CgNoticeSource.AgentSend;
                orderHK.OrderOutput.Waybill.EnterCode = OutEnterCode;

                orderHK.Fileitems = fileList;

                List<OrderItem> outorderItems = new List<OrderItem>();
                foreach (var item in productList)
                {
                ...
                orderHK.Orderitems = outorderItems;

                if (orderHK.Orderitems.Count() > 0)
                {
                    orderHK.Enter();

                    //生成出库通知并且锁定库存
                    JMessage message = orderHK.CgOutNotice().JsonTo<JMessage>();
                    if (!message.success)
                    {
                        orderHK.Abandon();
                    ...
                    {
                        orderHK.MainStatus = CgOrderStatus.待收货;
                        orderHK.StatusLogUpdate();
                    }
                    orderHK.OperateLog("香港发香港的发货订单保存成功");
                }
