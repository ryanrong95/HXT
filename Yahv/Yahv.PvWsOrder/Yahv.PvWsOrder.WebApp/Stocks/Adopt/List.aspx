<%@ Page Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="Yahv.PvOms.WebApp.Stocks.Adopt.List" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="server">
    ...
    <script>
        ...
        function Operation(val, row, index) {
            ...
            if (row.EnterCode == "") {
                button = ['<span class="easyui-formatted">',
                    , '<a class="easyui-linkbutton"  data-options="iconCls:\'icon-man\'" onclick="Adopt(\'' + row.ID + '\');return false;">认领</a> '
                    , '</span>'].join('');
            } else {
                button = ['<span class="easyui-formatted">',
                    , '<a class="easyui-linkbutton"  data-options="iconCls:\'icon-ok\'" onclick="Handle(\'' + row.ID + '\',\'' + row.EnterCode + '\');return false;">暂存处理</a> '
                    , '</span>'].join('');
            }
        ...
        //暂存处理
        function Handle(id,EnterCode) {
            $.myWindow({
                title: "暂存处理",
                width: 500,
                height: 350,
                url: location.pathname.replace('List.aspx', 'Handle.aspx?ID=' + id+'&EnterCode='+EnterCode),
                onClose: function () {
        ...
        function View(id) {
             $.myWindow({
                title: "认领",
                url: location.pathname.replace('List.aspx', 'File.aspx?ID=' + id),
                width: 1000,
                height: 800,
                onClose: function () {
                    window.grid.myDatagrid('flush');
