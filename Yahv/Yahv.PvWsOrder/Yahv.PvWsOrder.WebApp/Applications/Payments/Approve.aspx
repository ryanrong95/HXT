<%@ Page Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="Approve.aspx.cs" Inherits="Yahv.PvOms.WebApp.Applications.Payments.Approve" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="server">
    ...
    <script>
        ...
        $(function () {
            ...
            $("#btnPass").click(function () {
                ...
                $.messager.prompt('确认', "请您再次确认是否审批通过.", function (content) {
                    if (content) {
                        $.post('?action=Pass', { ID: ID, Content: content }, function (result) {
            ...
            $("#btnReject").click(function () {
                $.myWindow.setMyWindow("Approve", window);
                $.myWindow({
                    title: "审批驳回",
                    url: 'ApproveReject.aspx?ID=' + ID,
                    width: 550,
                    height: 300,
                    onClose: function () {
                        if (closeFlag == true) {
    ...
    <script>
        function Init() {
            if (model.ApplicationData != null) {
                ...
                $("#Receivable").text(Number(model.ReceiveData.TotalLeft).toFixed(4) + " " + model.ApplicationData.Currency);
                $("#Received").text(Number(model.ReceiveData.TotalRight).toFixed(4) + " " + model.ApplicationData.Currency);
                if (model.ApplicationData.Currency != "美元") {
                    $(".NotUsd").css("display", "table-row");
                    $("#ReceivableUSD").text(Number(model.ReceiveData.TotalLeftUSD).toFixed(4) + " 美元");
                    $("#ReceivedUSD").text(Number(model.ReceiveData.TotalRightUSD).toFixed(4) + " 美元");
                }
                $("#AvailableUSD").text(Number(model.ReceiveData.AvailableUSD).toFixed(4) + " 美元");
    ...
    <script>
        function ValidationOrder() {
            ...
            //20230626 张庆永：客户抵押RMB，收款RMB86766 ID612351，收到美金后退人民币，放开特殊权限
            if ((Number(model.ReceiveData.TotalLeftUSD) > Number(model.ReceiveData.AvailableUSD)) && model.AdminID != 'Admin00526') {
                $.messager.alert('提示', '应收美元金额 > 合计可用额度，不能审批通过');
                return false;
            }
            return true;
        }
        //返回列表页
        function BackToList() {
            var url = location.pathname.replace('Approve.aspx', 'ListApproval.aspx')
            window.location = url;
        }
        //添加合计行
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphForm" runat="server">
    <div class="easyui-layout" style="width: 100%; height: 100%; border: none">
        <div data-options="region:'north'," style="height: 40px; border: none; padding-top: 11px; padding-left: 5px">
            <a id="btnPass" class="easyui-linkbutton" iconcls="icon-ok">审批通过</a>
            <a id="btnReject" class="easyui-linkbutton" iconcls="icon-cancel">审批驳回</a>
        ...
        <div data-options="region:'west'," style="width: 30%; min-width: 360px; border: none; padding-left: 5px">
            <div class="easyui-panel" title="客户信息" style="border: none">
                <table class="liebiao">
                    ...
                    <tr class="NotUsd" style="display: none">
                        <td class="lbl" style="color: red">折算应收美元：</td>
                        <td style="color: red">
                            <label id="ReceivableUSD"></label>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr class="NotUsd" style="display: none">
                        <td class="lbl">折算实收美元：</td>
                        <td>
                            <label id="ReceivedUSD"></label>
                        </td>
                    </tr>
                    <tr>
                        <td class="lbl" style="color: red">合计可用额度：</td>
                        <td style="color: red">
                            <label id="AvailableUSD"></label>
                        </td>
                    </tr>
                </table>
            </div>
