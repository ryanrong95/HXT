<%@ Page Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="ListApproval.aspx.cs" Inherits="Yahv.PvOms.WebApp.Applications.Payments.ListApproval" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="server">
    ...
    <script>
        ...
        function Approve(index) {
            ...
            var url = location.pathname.replace('ListApproval.aspx', 'Approve.aspx?ID=' + data.ID)
            window.location = url;
            return false;
        }
        //详情
        function Detail(index) {
            var data = $("#tab1").myDatagrid('getRows')[index];
            var url = location.pathname.replace('ListApproval.aspx', 'Details.aspx?ID=' + data.ID + '&Source=2')
            window.location = url;
            return false;
        }
        //操作
        ...
        var Received = <%=Yahv.PvWsOrder.Services.Enums.ApplicationReceiveStatus.Received.GetHashCode()%>;
        function Operation(val, row, index) {
            var buttons = [];
            buttons.push('<span class="easyui-formatted">');
            if (row.ApplicationStatus == Examined && row.ApplicationStatus != Approved) {
                buttons.push('<a class="easyui-linkbutton"  data-options="iconCls:\'icon-man\'" onclick="Approve(' + index + ');return false;">经理审批</a> ');
            }
            else {
