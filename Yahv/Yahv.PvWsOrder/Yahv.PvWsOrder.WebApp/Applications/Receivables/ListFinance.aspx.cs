...
namespace Yahv.PvOms.WebApp.Applications.Receivables
{
    public partial class ListFinance : ErpParticlePage
    {
        ...
        Expression<Func<Application, bool>> Predicate()
        {
            ...
            predicate = predicate.And(item => item.ApplicationStatus != ApplicationStatus.Reject);
            predicate = predicate.And(item => item.ApplicationStatus != ApplicationStatus.Examining);
