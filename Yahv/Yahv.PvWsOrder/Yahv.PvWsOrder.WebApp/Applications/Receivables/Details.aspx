<%@ Page Language="C#" MasterPageFile="~/Uc/Works.Master" AutoEventWireup="true" CodeBehind="Details.aspx.cs" Inherits="Yahv.PvOms.WebApp.Applications.Receivables.Details" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="server">
    ...
    <script>
        function Init() {
            if (model.ApplicationData != null) {
                ...
                $("#Currency").text(model.ApplicationData.Currency);
                $("#AvailableBalance").text(model.BalanceData.AvailableBalance);
                $("#Receivable").text(model.ReceiveData.TotalLeft);
                $("#Received").text(model.ReceiveData.TotalRight);
                ...
                if (model.ApplicationData.PayerMethod == "支票") {
                    $(".check").css("display", "table-row");
                    $("#IsEntry").text(model.ApplicationData.IsEntry);
                    $("#CheckTitle").text(model.ApplicationData.CheckTitle);
                    if (model.ApplicationData.IsEntry == "否") {
                        $(".CheckDelivery").css("display", "table-row");
                        $("#CheckDelivery").text(model.ApplicationData.CheckDelivery);
                        if (model.ApplicationData.CheckDelivery == "送票") {
                            $(".CheckConsignee").css("display", "table-row");
                            $("#CheckConsignee").text(model.ApplicationData.CheckConsignee);
                        }
                        if (model.ApplicationData.CheckDelivery == "快递") {
                            $(".CheckConsignee").css("display", "table-row");
                            $("#CheckConsignee").text(model.ApplicationData.CheckConsignee);
                            $(".CheckCarrier").css("display", "table-row");
                            $("#CheckCarrier").text(model.ApplicationData.CheckCarrier);
                        }
                        if (model.ApplicationData.CheckDelivery == "倒入") {
                            $(".CheckPayeeAccount").css("display", "table-row");
                            $("#CheckPayeeAccount").text(model.ApplicationData.CheckPayeeAccount);
                        }
                    }
                };
            }
    ...
    <script>
        ...
        function BackToList() {
            var url
            if (Source == 1) {
                url = location.pathname.replace('Details.aspx', 'ListExamine.aspx')
            }
            if (Source == 2) {
                url = location.pathname.replace('Details.aspx', 'ListApproval.aspx')
            }
            if (Source == 3) {
                url = location.pathname.replace('Details.aspx', 'ListFinance.aspx')
            }
            window.location = url;
        }
        //添加合计行
        ...
        <div data-options="region:'west'," style="width: 30%; min-width: 360px; border: none; padding-left: 5px">
            <div class="easyui-panel" title="客户信息" style="border: none">
                <table class="liebiao">
                    ...
                    <tr>
                        <td class="lbl" style="color: red">实收总额：</td>
                        <td style="color: red">
            ...
            <div class="easyui-panel" title="客户付款人" style="border: none;">
                <table class="liebiao">
                    ...
                    <tr class="check" style="display: none">
                        <td class="lbl">是否入账：</td>
                        <td>
                            <label id="IsEntry"></label>
                        </td>
                    </tr>
                     <tr class="check" style="display: none">
                        <td class="lbl">支票抬头：</td>
                        <td>
                            <label id="CheckTitle"></label>
                        </td>
                    </tr>
                    <tr class="CheckDelivery" style="display: none">
                        <td class="lbl">送票方式：</td>
                        <td>
                            <label id="CheckDelivery"></label>
                        </td>
                    </tr>
                    <tr class="CheckConsignee" style="display: none">
                        <td class="lbl">送票地址：</td>
                        <td>
                            <label id="CheckConsignee"></label>
                        </td>
                    </tr>
                    <tr class="CheckCarrier" style="display: none">
                        <td class="lbl">送票承运商：</td>
                        <td>
                            <label id="CheckCarrier"></label>
                        </td>
                    </tr>
                    <tr class="CheckPayeeAccount" style="display: none">
                        <td class="lbl">倒入账户：</td>
                        <td>
                            <label id="CheckPayeeAccount"></label>
                        </td>
                    </tr>
                </table>
            </div>
