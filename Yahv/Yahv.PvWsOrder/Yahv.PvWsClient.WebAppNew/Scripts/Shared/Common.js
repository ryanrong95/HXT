...
//判断是否是香港银行
function checkIsHkBank(supplierBankName, supplierBankCode) {
    if (supplierBankName == null || supplierBankName == undefined || supplierBankName == '') {
        return true;
    }

    var ourBanks = ['BANK OF CHINA (HK) LTD', 'DBS Bank (Hong Kong) Limited'];
    if (supplierBankName.indexOf(ourBanks) > -1) {
        return true;
    }

    if (supplierBankCode == null || supplierBankCode == undefined
        || supplierBankCode == '' || supplierBankCode.length < 6) {
        // 没有 swift code
        return true;
    }

    if (supplierBankCode.substring(4, 6).toUpperCase() == 'HK') {
        return true;
    }
...
//计算手续费
function calcHandlingFee(handlingFeePayerType, supplierBankName, supplierBankCode) {
