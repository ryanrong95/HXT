  ...
  <div class="Warehousing">
    ...
    <div class="tablebox">
      <Table :columns="columns1" :data="data1" :loading="loading" :max-height="tableHeight" ref="table">
          ...
          <template slot-scope="{ row, index }" slot="TakerName">
             {{row.Consignee.TakerName}}
          </template>
          <template slot-scope="{ row, index }" slot="TakerPhone">
            <p v-if="row.Consignee!=undefined">{{row.Consignee.TakerPhone}}</p> 
          </template>
...
<script>
import{Delivery_Completed} from "../../api/Out"
export default {
  data() {
    return {
      tableHeight:500,
  ...
  methods: {
    ...
    searchbtn(){
      ...
      Delivery_Completed(data).then(res=>{
        console.log(res)
