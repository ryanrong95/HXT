...
{
    public class StorageOutNoticeService
    {
        /// <summary>
        /// 订单
        /// </summary>
        ...
        public string GenerateJson(string trackerID)
        {
            ...
            //通过 StorageID 找到对应的 ShelveID
            string[] storageIDs = this.OrderItems.Select(t => t.StorageID).ToArray();
            Dictionary<string, string> mapStorageIDvsShelveID = new Dictionary<string, string>();
            using (PsOrderRepository repository = new PsOrderRepository())
            {
                mapStorageIDvsShelveID = repository.ReadTable<Layers.Data.Sqls.PsOrder.StoragesTopView>()
                                            .Where(t => storageIDs.Contains(t.ID))
                                            .Select(item => new
                                            {
                                                StorageID = item.ID,
                                            ...
                                            .ToArray()
                                            .ToDictionary(item => item.StorageID, item => item.ShelveID);
            }
            ...
            var items = this.OrderItems.Select(item => new StorageNoticeModel.Item
            {
                ...
                ShelveID = mapStorageIDvsShelveID.Where(t => t.Key == item.StorageID).FirstOrDefault().Value,
            }).ToArray();
            ...
            //var fileInfos = this.FileInfos.Select(item => new StorageNoticeModel.File
            //{
            ...
            StorageNoticeModel.Transport consignee = null;
            if (this.OrderTransport.TransportMode == (int)TransportMode.Express || this.OrderTransport.TransportMode == (int)TransportMode.Dtd)
            {
                consignee = new StorageNoticeModel.Transport
                {
                    ...
                    ExpressEscrow = this.OrderTransport.ExpressEscrow,
                    ...
                    TakerIDType = "1", //这里给默认值是因为接收端那边的转换要求

                    ExpressTransport = this.OrderTransport.ExpressTransport,
                };
            }
            else if (this.OrderTransport.TransportMode == (int)TransportMode.PickUp)
            {
                ...
                using (PsOrderRepository repository = new PsOrderRepository())
                {
                    var thePicker = repository.ReadTable<Layers.Data.Sqls.PsOrder.Pickers>().Where(t => t.ID == this.OrderTransport.PickerID).FirstOrDefault();
                    if (thePicker != null)
                    {
                        idCode = thePicker.IDCode;
                        idType = thePicker.IDType;
                    }
                }
                ...
                consignee = new StorageNoticeModel.Transport
                {
                    ...
                    TakerName = this.OrderTransport.Contact, //this.Picker.Contact,
                    TakerPhone = this.OrderTransport.Phone, //this.Picker.Phone,
                    TakerIDCode = idCode, //this.Picker.IDCode,
                    TakerIDType = Convert.ToString(idType), //Convert.ToString(this.Picker.IDType),
                };
            }
        ...
        public string GenerateJsonFile()
        {
            ...
            var packingFiles = this.FileInfos.Select(item => new StorageNoticeFileModel.File
            {
                Type = item.Type.ToString(),
                CustomName = item.CustomName,
                Url = string.Join(@"/", fileUrlPrefix, item.Url),
        ...
        public void SendNotice(string orderID)
        {
            ...
            string url = string.Join(@"/", warehouseUrl, "OutNotices/Enter");
            ...
            using (PsOrderRepository repository = new PsOrderRepository())
            {
                repository.Insert(new Layers.Data.Sqls.PsOrder.Logs
                {
                    ...
                    ActionName = LogAction.NewStorageOutSendNotice.GetDescription(),
                    MainID = orderID,
                    Url = url,
