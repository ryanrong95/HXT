...
(function ($) {
    //接口地址
    var ajaxUrl = {
        getSupplierPayees: '/crmplus2api/Suppliers/Payees',
    }
    ...
    innerMethods.prototype = {
        ...
        //ajax请求供应商收款人数据
        getSupplierPayees: function (sender, value, success) {
            sender.combobox('clear');
            sender.combobox('loadData', []);

            $.ajax({
                url: ajaxUrl.getSupplierPayees,
                dataType: 'json',
                type: 'GET',
                data: { name: value },
                ...
                error: function (err) {
                    alert('supplierPayee.error:' + JSON.stringify(err));
                }
            });
        },
    ...
    $.fn.supplierPayeeCrmPlus = function (opt, param) {
        //如果options为string，则是方法调用
        if (typeof opt == 'string') {
            var method = $.fn.supplierPayeeCrmPlus.methods[opt];
            if (method) {
                return method(this, param);//如果有该方法就调用该方法
            } else {
                alert("supplierPayee.插件没有这个方法: " + opt)
            }
        }

        var options = opt || {};
        options = $.extend(true, {}, $.fn.supplierPayeeCrmPlus.defaults, options);
        ...
        return this.each(function () {
            ...
            if (!sender_id) {
                sender_id = 'supplierPayee_sender_' + Math.random().toString().substring(2);
            ...
            if (!formName) {
                alert('supplierPayee.控件name不能为空!');
            }

            sender.combobox({
                loader: function (param, success, error) {
                    if (options.supplierName) {
                        myMethods.getsupplierPayees(sender, options.supplierName, success);
                    }
                },
                mode: 'remote',
                valueField: 'ID',
    ...
    //插件的默认配置
    $.fn.supplierPayeeCrmPlus.defaults = {
        prompt: '请选择供应商收款人',
        required: true,
        supplierName: '',
        onSelect: function (record) {
    ...
    //插件对外的方法
    $.fn.supplierPayeeCrmPlus.methods = {
        //获取数据
        getData: function (jq) {
            var sender = $(jq);
        ...
        //设置供应商名称
        setSupplierName: function (jq, name) {
            var sender = $(jq);
            var myMethods = new innerMethods();
            myMethods.getSupplierPayees(sender, name);
        },
    };

    //将自定义的插件加入 easyui 的插件组
    $.parser.plugins.push('supplierPayeeCrmPlus');
})(jQuery)