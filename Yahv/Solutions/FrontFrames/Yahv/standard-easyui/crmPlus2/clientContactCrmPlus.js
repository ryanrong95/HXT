...
(function ($) {
    //接口地址
    var ajaxUrl = {
        getClientContacts: '/crmplus2api/Clients/Contacts',
    }
    ...
    $.fn.clientContactCrmPlus = function (opt, param) {
        //如果options为string，则是方法调用
        if (typeof opt == 'string') {
            var method = $.fn.clientContactCrmPlus.methods[opt];
            if (method) {
        ...
        var options = opt || {};
        options = $.extend(true, {}, $.fn.clientContactCrmPlus.defaults, options);
    ...
    //插件的默认配置
    $.fn.clientContactCrmPlus.defaults = {
        prompt: '请选择客户联系人',
        required: true,
        clientID: '',
        onSelect: function (record) {
    ...
    //插件对外的方法
    $.fn.clientContactCrmPlus.methods = {
        //获取数据
        getData: function (jq) {
            var sender = $(jq);
    ...
    $.parser.plugins.push('clientContactCrmPlus');
})(jQuery)