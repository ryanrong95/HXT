...
(function ($) {
    //接口地址
    var ajaxUrl = {
        getCompanyPayees: '/crmplus2api/Companies/Payees',
    }
    ...
    $.fn.companyPayeeCrmPlus = function (opt, param) {
        //如果options为string，则是方法调用
        if (typeof opt == 'string') {
            var method = $.fn.companyPayeeCrmPlus.methods[opt];
            if (method) {
        ...
        var options = opt || {};
        options = $.extend(true, {}, $.fn.companyPayeeCrmPlus.defaults, options);
            ...
            sender.combobox({
                loader: function (param, success, error) {
                    if (options.companyID) {
                        myMethods.getCompanyPayees(sender, options.companyID, success);
                    }
                },
                mode: 'remote',
                valueField: 'ID',
    ...
    //插件的默认配置
    $.fn.companyPayeeCrmPlus.defaults = {
        prompt: '请选择公司收款人',
        required: true,
        companyID: '',
        onSelect: function (record) {
    ...
    //插件对外的方法
    $.fn.companyPayeeCrmPlus.methods = {
        //获取数据
        getData: function (jq) {
            var sender = $(jq);
        ...
        setCompanyID: function (jq, id) {
            var sender = $(jq);
            var myMethods = new innerMethods();
            myMethods.getCompanyPayees(sender, id);
        },
    };

    //将自定义的插件加入 easyui 的插件组
    $.parser.plugins.push('companyPayeeCrmPlus');
})(jQuery)