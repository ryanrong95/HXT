...
(function ($) {
    ...
    //接口地址
    var AjaxUrl = {
        getCompanys: '/crmplus2api/Enterprises/Companies'
    }

    //编写companyCrmPlus插件
    $.fn.companyCrmPlus = function (opt, param) {
        //如果options为string，则是方法调用
        if (typeof opt == 'string') {
            var method = $.fn.companyCrmPlus.methods[opt];
            if (method) {
        ...
        var options = opt || {};
        options = $.extend(true, {}, $.fn.companyCrmPlus.defaults, options);
        ...
        return this.each(function () {
            ...
            sender.combobox({
                loader: function (param, success, error) {
                    ...
                    setTimeoutAjax = setTimeout(function () {
                        $.ajax({
                            ...
                            error: function (err) {
                                alert('companyCrmPlus.error:' + JSON.stringify(err));
                            }
                        });
                    }, 100);
                },
                ...
                validType: 'companyCrmPlus["' + sender_id + '"]',
            });
    ...
    $.parser.plugins.push('companyCrmPlus');
})(jQuery)