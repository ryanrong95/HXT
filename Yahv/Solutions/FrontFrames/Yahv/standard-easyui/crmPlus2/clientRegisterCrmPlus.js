...
(function ($) {
    ...
    $.extend($.fn.validatebox.defaults.rules, {
        client: {
            validator: function (value, param) {
                var sender = $('#' + param[0]);
                var data = sender.clientRegisterCrmPlus('getData');
                if (data == null) {
                    return false;
                }
    ...
    //接口地址
    var ajaxUrl = {
        getClients: '/crmplus2api/Enterprises/MyClients',
        getNewRegistered: '/crmplus2api/Clients/NewRegistered'
    }

    //客户快速注册页面地址
    var pageUrl = {
        clientQuickRegister: '/crmplus2/Crm/Client/QuickRegister.aspx'
    }
    ...
    //编写clientRegister插件
    $.fn.clientRegisterCrmPlus = function (opt, param) {
        //如果options为string，则是方法调用
        if (typeof opt == 'string') {
            var method = $.fn.clientRegisterCrmPlus.methods[opt];
            if (method) {
        ...
        var options = opt || {};
        options = $.extend(true, {}, $.fn.clientRegisterCrmPlus.defaults, options);
        ...
        return this.each(function () {
            ...
            sender.combobox({
                loader: function (param, success, error) {
                    ...
                    setTimeoutAjax = setTimeout(myMethods.getClients(sender, q, success), 100);
                },
                mode: 'remote',
                valueField: 'ID',
                textField: 'Name',
    ...
    //插件的默认配置
    $.fn.clientRegisterCrmPlus.defaults = {
        prompt: '请输入两位以上搜索客户',
        required: true,
    ...
    //插件对外的方法
    $.fn.clientRegisterCrmPlus.methods = {
        //获取数据
        getData: function (jq) {
            var sender = $(jq);
    ...
    $.parser.plugins.push('clientRegisterCrmPlus');
})(jQuery);