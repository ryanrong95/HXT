...
(function ($) {
    ...
    $.fn.assignment = function (opt, param) {
        ...
        var $selected = $("<table style='display:none;'></table>");
        ...
        var $div = $("<div style='height:30px;display:none'></div>");//两个列表之间的间距
        $(sender).data("div", $div);
                ...
                onUnselect: function (index, row) {
                    ...
                    for (var i = 0; i < selectselectedData.length; i++) {
                        if (selectselectedData[i].ID != row.ID) {
                            selectedData.push(selectselectedData[i]);
                        }
                    }
                    $(sender).data("selectselectedData", selectedData);
                },
    ...
    $.fn.assignment.defaults = {
        type: null,// "holistic"整体性的指派，null就代表指派某个
        columns: null,
        //待选择列表
        pending: {
            title: '待选择列表',
