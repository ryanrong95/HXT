...
(function ($) {
    ...
    //获取换算结果以及汇率
    function getPriceResult(from, to, price, cb) {
        $.ajax({
            type: "get",
    ...
    function doGetRate(sender) {
        ...
        getRate(currency, currency1, function (data) {
    ...
    function dosomething(sender, price, flag) {
        ...
        var currency = $(sender).data("options").currency;
        var currency1 = $(sender).data("options").currency1;
        getPriceResult(currency, currency1, digitalConversion(price), function (data) {
            ...
            if (flag == 1 || flag == 3) {
                ...
                var TaxedPrice = data.Price;
                if (options.currency == 'CNY') {
                    TaxedPrice = ConvertToTaxPrice(options.invoiceType, Number(data.Price));
                }
            ...
            if (flag == 2) {
                ...
                var TaxedPrice = Price1Val;
                if (options.currency == 'CNY') {
                    TaxedPrice = ConvertToTaxPrice(options.invoiceType, digitalConversion(price));
                }
    ...
    function CurrencyConversion(sender, TaxedPrice) {
        ...
        if (TaxedPrice == "" || TaxedPrice == "0.00000") {
            ...
            if (options.currency == 'CNY') {
                ...
                getPriceResult(options.currency, options.currency1, digitalConversion(Price1), function (data) {
                    setAllRate(sender, data.Rate);
