...
(function ($) {
    ...
    $.myDialogFuse = function (options) {
        ...
        var options = $.extend({}, $.myDialogFuse.defaults, options);
        ...
        var html = ['<iframe id="', id, '" class="easyui-myDialogFuse" style="',
        ...
        $.myDialogFuse.options.push(options);

        return null;
    };

    if (!top.$.myDialogFuse.options) {
        top.$.myDialogFuse.options = [];
    }

    $.myDialogFuse.defaults = {
        isHaveOk: true,
        isHaveCancel: true,
        modal: true,
        collapsible: false,
    ...
    $.myDialogFuse.close = function () {
        try {
            // todo 未触发close事件
            var $ = top.window.$;
            var edialog = $('.easyui-myDialogFuse').last();
