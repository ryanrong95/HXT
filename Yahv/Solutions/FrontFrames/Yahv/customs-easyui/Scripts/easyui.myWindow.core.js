...
(function ($) {
    ...
    $.myWindowCore = function (options) {
        ...
        var options = $.extend({}, $.myWindowCore.defaults, options);
        ...
        var writer = setInterval(function () {
            ...
            if (doc.body) {
                ...
                script.src = 'http://fix.szhxd.net/frontframe/customs-easyui/controls/topContentWindow.core.js';
        ...
        options.iframeId = id;
        top.$.myWindowCore.options.push(options);
        return null;
    };

    if (!top.$.myWindowCore.options) {
        top.$.myWindowCore.options = [];
    }

    $.myWindowCore.defaults = {
    ...
    $.myWindowCore.close = function () {
        try {
            // todo 未触发close事件
            var $ = top.window.$;
            var edialog = $('.easyui-myWindowCore').last();
