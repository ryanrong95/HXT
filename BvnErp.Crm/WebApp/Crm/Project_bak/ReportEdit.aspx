<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ReportEdit.aspx.cs" Inherits="WebApp.Crm.Project_bak.ReportEdit" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            var editor2 = UM.getEditor("editor2");
            ...
            if (alldata != null) {
                if (alldata["Content"] != null) {
                    UM.getEditor('editor1').setContent(escape2Html(alldata["Content"]));
                }
                if (alldata["Content"] != null) {
                    UM.getEditor('editor2').setContent(escape2Html(alldata["Plan"]));
                }
                $("#OriginalStaffs").textbox("setValue", alldata["OriginalStaffs"]);
                $("#Type").combobox("setValue", alldata["Type"]);
                $("#NextType").combobox("setValue", alldata["NextType"]);
            ...
            $("#NextType").combobox("textbox").bind("blur", function () {
                var value = $("#NextType").combobox("getValue");
                var data = $("#NextType").combobox("getData");
                var valuefiled = $("#NextType").combobox("options").valueField;
                var index = $.easyui.indexOfArray(data, valuefiled, value);
                if (index < 0) {
                    $("#NextType").combobox("clear");
                }
            });
        });
        ...
        //保存校验
        function Save() {
            var content1 = UM.getEditor('editor1').getContent();
            var content2 = UM.getEditor('editor2').getContent();
            if (content1 == "") {
                $.messager.alert('提示', '请输入跟进内容！');
                return false;
            }
            if (content2 == "") {
                $.messager.alert('提示', '请输入行动计划！');
                return false;
            }
            ...
            $("#editorValue1").val(content1);
            $("#editorValue2").val(content2);
    ...
    <form id="form1" runat="server" method="post" enctype="multipart/form-data" >
        <table id="table1" style="width: 100%; height: 100%">
            <tr style="height: 50px">
                ...
                <td>
                    <input class="easyui-combobox" id="Type" name="Type" style="width: 180px"
                        data-options="valueField:'value',textField:'text',required:true,data: TypeData, editable:false, panelMaxHeight:'100px'," />
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td colspan="3">
                    <input id="editorValue1" type="hidden" name="editorValue1" />
                    <script id="editor1" type="text/plain" style="width: 99%; height: 200px;"></script>
                </td>
            </tr>
            <tr style="height: 50px">
                ...
                <td class="lbl" style="width: 80px">下次跟进方式</td>
                <td>
                    <input class="easyui-combobox" id="NextType" name="NextType" style="width: 180px"
                        data-options="valueField:'value',textField:'text',required:true,data: TypeData, editable:false, panelMaxHeight:'100px'," />
                </td>
            </tr>
            <tr>
                <td class="lbl" style="width: 80px">行动计划</td>
                <td colspan="3">
                    <input id="editorValue2" type="hidden" name="editorValue2" />
                    <script id="editor2" type="text/plain" style="width: 99%; height: 200px;"></script>
                </td>
            </tr>
            <tr style="height: 50px">
