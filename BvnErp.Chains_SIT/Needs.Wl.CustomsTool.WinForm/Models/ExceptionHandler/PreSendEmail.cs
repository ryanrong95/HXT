...
namespace Needs.Wl.CustomsTool.WinForm.Models.ExceptionHandler
{
    ...
    public class PreSendEmail : AbstractHandler
    {
        public override void Handle(ContextParam contextParam)
        {
            ...
            Needs.Ccs.Services.Models.BalanceQueueRedis.BalanceQueueInfo targetInfo = null;

            if (minProcessIDModelInCircle != null && minProcessIDModelInCircle.FilePath.ToLower().EndsWith(".xml"))
            {
                targetInfo = minProcessIDModelInCircle;
            }
            else if (contextParam.PairModelInCircleAndQueue != null && contextParam.PairModelInCircleAndQueue.FilePath.ToLower().EndsWith(".xml"))
            {
                targetInfo = contextParam.PairModelInCircleAndQueue;
            }

            targetInfo.ProcessStatus = Needs.Ccs.Services.Enums.BalanceQueueProcessStatus.PreSendEmail;
            targetInfo.UpdateDate = DateTime.Now;
            contextParam.Redis.ListRightPush<BalanceQueueInfo>(contextParam.RedisKey.PreSendEmailList, targetInfo);
        }
    }
}
