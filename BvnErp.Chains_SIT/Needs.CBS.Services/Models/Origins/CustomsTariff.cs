    ...
    public class CustomsTariff : IUnique, IPersist, IFulError, IFulSuccess
    {
        ...
        public IEnumerable<CustomsOriginTariff> OriginTariffs
        {
            get
            {
                ...
                {
                    using (var view = new Views.Alls.CustomsOriginTariffsAll())
                    {
                        this.originTariffs = view.Where(item => item.CustomsTariffID == this.ID).ToList();
                    }
                }
        ...
        private IEnumerable<CustomsElementsDefault> elementsDefault;

        /// <summary>
        /// 申报要素默认值
        /// </summary>
        public IEnumerable<CustomsElementsDefault> ElementsDefaults
        {
            get
            {
                if (elementsDefault == null)
                {
                    using (var view = new Views.Alls.CustomsElementsDefaultsAll())
                    {
        ...
        public void Enter()
        {
            ...
            {
                int count = reponsitory.ReadTable<Layer.Data.Sqls.Customs.CustomsTariffs>().Count(item => item.ID == this.ID);

                if (count == 0)
                {
                    reponsitory.Insert(new Layer.Data.Sqls.Customs.CustomsTariffs
                    {
                        ID = this.ID,
                        HSCode = this.HSCode,
                        Name = this.Name,
                ...
                {
                    reponsitory.Update<Layer.Data.Sqls.Customs.CustomsTariffs>(new
                    {
                        ID = this.ID,
                        HSCode = this.HSCode,
                        Name = this.Name,
