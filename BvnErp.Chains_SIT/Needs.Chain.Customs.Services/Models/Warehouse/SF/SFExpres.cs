        ...
        public string RequestImage(List<WaybillDto> waybillDtoList)
        {
            try
            {
                ...
                Needs.Ccs.Services.Models.DeliveryNoticeApiLog apiLog = new Needs.Ccs.Services.Models.DeliveryNoticeApiLog()
                {
                    ...
                    OrderID = waybillDto.mailNo,
                    TinyOrderID = "",
                    Url = reqURL,
                    RequestContent = jsonParam,
                ...
                fileDic.SetChildFolder(Needs.Ccs.Services.SysConfig.SF);
                fileDic.CreateDataDirectory();

                generateImage(apiResultData.FirstOrDefault().ToString(), fileDic.FilePath);

                return fileDic.FileUrl;
            }
            catch(Exception ex)
            {
                return null;
