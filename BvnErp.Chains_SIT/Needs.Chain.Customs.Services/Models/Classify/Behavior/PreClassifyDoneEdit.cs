    ...
    public sealed class PreClassifyDoneEdit : Classify
    {
        ...
        private void WriteProductClassifyLog(PreClassifyProduct classifyProduct, Layer.Data.Sqls.ScCustomsReponsitory reponsitory)
        {
            ...
            if (declarant != null)
            {
                ...
                string summary = "报关员【" + declarant.RealName + "】完成了预归类中已完成的归类。归类结果:" +
        ...
        private void InsertProductCategory(PreClassifyProduct classifyProduct, Layer.Data.Sqls.ScCustomsReponsitory reponsitory)
        {
            var productCategory = new ProductCategory
            {
                ID = ChainsGuid.NewGuidUp(),
                Model = classifyProduct.Model,
                Name = classifyProduct.ProductName,
                HSCode = classifyProduct.HSCode,
                TariffRate = classifyProduct.TariffRate ?? 0,
                AddedValueRate = classifyProduct.AddedValueRate ?? 0,
                UnitPrice = classifyProduct.PreProduct.Price,
                InspectionFee = classifyProduct.IsInsp ? (decimal?)classifyProduct.InspectionFee : null,
                Elements = classifyProduct.Elements,
        ...
        private void UpdateProductTaxCategory(PreClassifyProduct classifyProduct, Layer.Data.Sqls.ScCustomsReponsitory reponsitory)
        {
