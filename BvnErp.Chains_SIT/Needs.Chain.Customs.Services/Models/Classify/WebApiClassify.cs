...
namespace Needs.Ccs.Services.Models
{
    ...
    /// WebApi的归类视图
    /// </summary>
    public class WebApiClassify : IUnique
    {
        public string ID
        {
            get;set;
        ...
        public bool IsCCC
        {
            get
            {
                using (var view = new Views.ProductControlsView(this.Model))
                {
                    int count = view.Count(item => item.Type == Enums.ProductControlType.CCC);
                    if (count == 0)
                    {
                        return false;
                    }
