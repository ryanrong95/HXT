...
{
    public class AccWordNo : IWordNo
    {
        public void GetWordNo()
        {
            var query = new Needs.Ccs.Services.Views.AccImportView().
                Where(item => item.Status == Needs.Ccs.Services.Enums.Status.Normal && item.ExchangeDate != null && item.AccCreSta == true);
            List<string> SwapIDs = query.Where(t => t.AccCreWord == null || t.AccCreWord == "").Select(t => t.Code).ToList();
            ...
            wordNoRequest.归属方案编号 = MakeAccountSetting.AccImport_归属方案编号;
                    ...
                    if (item.生成凭证 == 1)
                    {
                        using (Layer.Data.Sqls.ScCustomsReponsitory responsitory = new Layer.Data.Sqls.ScCustomsReponsitory())
                        {
                            responsitory.Update<Layer.Data.Sqls.ScCustoms.MoneyOrders>(
                                       new
                                       {
                                           AccCreWord = item.FNumber,
                                           AccCreNo = item.FSerialNum
                                       }, t => t.Code == item.标识);
                        }
                    }
                }
