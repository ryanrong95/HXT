...
{
    public class InvoiceWordNoFull : IWordNo
    {
        public void GetWordNo()
        {
            var query = new Needs.Ccs.Services.Views.InvoiceCredentialView().
               Where(item => item.Status == Needs.Ccs.Services.Enums.Status.Normal
                       && item.InvoiceType == Needs.Ccs.Services.Enums.InvoiceType.Full
                       && item.InvoiceNo != null
                       && item.InCreSta == true);
            List<string> SwapIDs = query.Where(t => t.InCreWord == null || t.InCreWord == "").Select(t => t.ID).ToList();
            ...
            wordNoRequest.归属方案编号 = MakeAccountSetting.FullInvoImport_归属方案编号;
