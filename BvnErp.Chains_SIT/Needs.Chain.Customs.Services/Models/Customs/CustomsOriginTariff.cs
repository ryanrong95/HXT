    ...
    public class CustomsOriginTariff : IUnique, IPersist, IPersistence
    {
        ...
        public void Enter()
        {
            try
            {
                ...
                {
                    int count = reponsitory.ReadTable<Layer.Data.Sqls.ScCustoms.CustomsOriginTariffs>().Count(item => item.ID == this.ID);
                    if (count == 0)
                    {
                        reponsitory.Insert(new Layer.Data.Sqls.ScCustoms.CustomsOriginTariffs
                        {
                    ...
                    {
                        reponsitory.Update(new Layer.Data.Sqls.ScCustoms.CustomsOriginTariffs
                        {
                            ID = this.ID,
                            CustomsTariffID = this.CustomsTariffID,
                            Origin = this.Origin,
