...
{
    public class PayExchangeToYahvReceivableView
    {
        ScCustomsReponsitory Reponsitory { get; set; }

        public PayExchangeToYahvReceivableView()
        {
            this.Reponsitory = new ScCustomsReponsitory();
        }

        public List<PayExchangeToYahvReceivableViewModel> GetData(string payExchangeApplyID)
        {
            var payExchangeApplies = this.Reponsitory.ReadTable<Layer.Data.Sqls.ScCustoms.PayExchangeApplies>()
                                .Where(t => t.PayExchangeApplyStatus == (int)Enums.PayExchangeApplyStatus.Audited);
            //.Where(t => t.PayExchangeApplyStatus == (int)Enums.PayExchangeApplyStatus.Approvaled
            //         || t.PayExchangeApplyStatus == (int)Enums.PayExchangeApplyStatus.Completed);
            ...
            var linq = from payExchangeApply in payExchangeApplies
                       ...
                       select new PayExchangeToYahvReceivableViewModel
                       {
    ...
    public class PayExchangeToYahvReceivableViewModel
    {
        /// <summary>
        /// 金额
        /// </summary>
