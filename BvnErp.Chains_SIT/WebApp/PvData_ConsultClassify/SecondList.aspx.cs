...
namespace WebApp.PvData_ConsultClassify
{
    ...
    public partial class SecondList : Uc.PageBase
    {
        ...
        protected void data()
        {
            ...
            Expression<Func<PD_PreClassifyProduct, bool>> expression = item => item.ClassifyStatus == ClassifyStatus.First && item.PreProduct.UseType == PreProductUserType.Consult;
        ...
        private string GetClassifiedResults(PD_PreClassifyProduct[] classifyProducts)
        {
            ...
            return classifyProducts.Select(item => new
            {
                ...
                HSCode = item.HSCode,
                TariffName = item.ProductName,
                ImportPreferentialTaxRate = item.TariffRate?.ToString("0.0000"),//芯达通没有区分优惠税率和加征税率, 将进口关税赋给优惠税率
                VATRate = item.AddedValueRate?.ToString("0.0000"),
                ExciseTaxRate = item.ExciseTaxRate?.ToString("0.0000") ?? "0.0000",
                TaxCode = item.TaxCode,
                TaxName = item.TaxName,
