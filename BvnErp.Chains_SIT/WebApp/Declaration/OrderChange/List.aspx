<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="WebApp.Declaration.OrderChange.List" %>
...
<head runat="server">
  ...
  <%--  <script>
    ...
    <script type="text/javascript">
        ...
        function EditFee(ID) {
            var url = location.pathname.replace(/List.aspx/ig, 'Edit.aspx') + '?ID=' + ID + '&From=EditDeclareChange';;
            window.location = url;
        }
        ...
        function Operation(val, row, index) {
            ...
            if (row.Type ==<%=Needs.Ccs.Services.Enums.OrderChangeType.TaxChange.GetHashCode()%>) {
                buttons = '<a href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:3px" onclick="EditFee(\'' + row.OrderID + '\')" group >' +
                    ...
                    '<span class="l-btn-text">修改税费</span>' +
