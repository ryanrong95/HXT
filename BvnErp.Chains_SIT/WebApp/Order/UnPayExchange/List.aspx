<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="WebApp.Order.UnPayExchange.List" %>
    ...
    <script type="text/javascript">
        ...
        function Search() {
            ...
            $('#orders').myDatagrid('search', {
                OrderID: orderID, ClientCode: clientCode, Currency: currency, OrderStatus: orderStatus, StartDate: startDate, EndDate: endDate, ClientType: type,
                ClientID: clinetID,
            });
        }
        ...
        //查看订单详情
        function View(id) {
            var url = location.pathname.replace(/List.aspx/ig, '../Detail2.aspx') + '?ID=' + id;
        ...
        function ViewPaid(ID) {
            var url = location.pathname.replace(/List.aspx/ig, 'PaidDetail.aspx?ID=' + ID);
            top.$.myWindow({
                iconCls: "",
                url: url,
                noheader: false,
        ...
        function Operation(val, row, index) {
            var buttons = '<a href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:3px" onclick="ApplyPay(' + index + ')" group >' +
                '<span class =\'l-btn-left l-btn-icon-left\'>' +
                '<span class="l-btn-text">申请付汇</span>' +
        ...
        function ExtraPEDateFT(val, row, index) {
            var span = '<span ' + ((row.ExtraPEDate != '-' && Number(row.ExtraPEDate) < 10) ? 'style="color:red"' : '') + ' >' + row.ExtraPEDate + '</span>';
            return span;
        }
        ...
        function calcSomeSum(rows) {
            ...
            for (var i = 0; i < rows.length; i++) {
                var currentTotalDeclarePrice = Number(Number(rows[i].UnpaidAmount).toFixed(2));
        ...
        function isAllowPayment(row) {
            if (row.IsPrePayExchange) {
                ...
                if (row.DeclareDate == null) {
                    ...
                    //var minusDays = getMinusDays(curDate, row.DeclareDate);
                    //if (minusDays > 365) {
                    //    $.messager.alert('提示', '客户(' + row.ClientCode + ')订单(' + row.ID + ')已报关超过一年，无法换汇！');
                    //    return false;
                    //} else {
                    //    return true;
                    //}
            ...
            <ul>
                <li>
                    ...
                    <a id="btnReset" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-redo'" onclick="Reset()">重置</a>
                     <span id="sum-container" style="margin-left: 55px;color:red;">
                        <label>合计</label>
                        <label style="margin-left: 25px;">未换汇总金额:</label>
