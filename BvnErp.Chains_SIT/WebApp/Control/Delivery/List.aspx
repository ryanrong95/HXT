<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="WebApp.Control.Delivery.List" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            //代理订单列表初始化
            $('#orders').myDatagrid({
                border: false, nowrap: false, fitColumns: true, fit: true, toolbar: '#topBar',
        ...
        function Search() {
            ...
            $('#orders').myDatagrid('search', {
                OrderID: orderID, ClientCode: clientCode, StartDate: startDate, EndDate: endDate, ClientType: type,
                ClientID: clinetID,
            });
        }
        ...
        //查看订单详情
        function View(id) {
            var url = location.pathname.replace(/List.aspx/ig, '../../Order/MainDetail.aspx') + '?ID=' + id;
        ...
        //解绑订单垫资
        function ToDeal(AdvanceRecordID) {
            $.messager.confirm('确认', '是否确认处理！', function (success) {
                if (success) {
                    $.post('?action=ToDeal', { AdvanceRecordID: AdvanceRecordID }, function (res) {
        ...
        function Operation(val, row, index) {
            var msg = row.HasNotified ? '已通知出库' : '解绑订单垫资';
            var icon = row.HasNotified ? 'icon-ok' : 'icon-edit';

            var buttons = '<a href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:3px" onclick = "View(\'' + row.MainOrderID + '\')" group >' +
                '<span class =\'l-btn-left l-btn-icon-left\'>' +
                '<span class="l-btn-text">查看</span>' +
                '<span class="l-btn-icon icon-search">&nbsp;</span>' +
                '</span>' +
                '</a>';
            buttons += '<a href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:3px" onclick="ToDeal(\'' + row.AdvanceRecordID + '\')" group >' +
                '<span class =\'l-btn-left l-btn-icon-left\'>' +
                '<span class="l-btn-text">' + msg + '</span>' +
...
<body class="easyui-layout">
        ...
        <div id="search">
            <ul>
                <li>
                    ...
                    <input class="easyui-textbox" id="OrderID" data-options="validType:'length[1,50]'" style="width: 280px;" />
                    <span class="lbl">客户编号: </span>
                    <input class="easyui-textbox" id="ClientCode" data-options="validType:'length[1,50]'" style="width: 280px;" />
                    <br />
    ...
    <div id="data" data-options="region:'center',border:false">
        <table id="orders" title="待发货">
            <thead>
                <tr>
                    <th data-options="field:'ID',align:'left'" style="width: 150px">订单编号</th>
                    <th data-options="field:'ClientCode',align:'left'" style="width: 150px">客户编号</th>
                    <th data-options="field:'ClientName',align:'left'" style="width: 200px">客户名称</th>
                    <th data-options="field:'DeclarePrice',align:'center'" style="width: 10%;">订单总货值</th>
                    <th data-options="field:'Currency',align:'center'" style="width: 120px">币种</th>
                    <th data-options="field:'Amount',align:'center'" style="width: 10%;">垫资金额（RMB）</th>
                    <th data-options="field:'CreateDate',align:'center'" style="width: 120px">下单日期</th>
