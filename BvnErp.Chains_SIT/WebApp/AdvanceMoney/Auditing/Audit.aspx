<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Audit.aspx.cs" Inherits="WebApp.AdvanceMoney.Auditing.Audit" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        //查看详情
        function View(ID) {
            var url = location.pathname.replace(/Audit.aspx/ig, '../View.aspx')
                + '?From=Manager&ID=' + ID;
            top.$.myWindow({
                iconCls: "",
                url: url,
        ...
        function Auditing(Index) {
            ...
            if (rowdata) {
                ...
                $.post('?action=CheckIsExcced', {
                    ...
                    if (rel.success) {
                        //小于100W或有权限
                        var url = location.pathname.replace(/Audit.aspx/ig, '../View.aspx') + "?From=Manager&ID=" + rowdata.ID;
                        top.$.myWindow({
                            iconCls: "",
                            url: url,
                            noheader: false,
        ...
        function Operation(val, row, index) {
            if (row.IntStatus == '<%=Needs.Ccs.Services.Enums.AdvanceMoneyStatus.Effective.GetHashCode()%>'
                ...
                || row.IntStatus == '<%=Needs.Ccs.Services.Enums.AdvanceMoneyStatus.RiskAuditing.GetHashCode()%>') {
        ...
        function ViewPaidDetail(ClientID) {
            var url = location.pathname.replace(/Audit.aspx/ig, '../AdvanceRecord.aspx')
                + '?ClientID=' + ClientID;
            top.$.myWindow({
                iconCls: "",
                url: url,
