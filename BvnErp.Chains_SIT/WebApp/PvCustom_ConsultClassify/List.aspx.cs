...
namespace WebApp.PvCustom_ConsultClassify
{
    public partial class List : Uc.PageBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            var classifyStatus = EnumUtils.ToDictionary<Needs.Ccs.Services.Enums.ClassifyStatus>().Select(item => new { item.Key, item.Value });
            this.Model.ClassifyStatus = classifyStatus.Json();
        }
        ...
        protected void data()
        {
            ...
            string classifyStatus = Request.QueryString["ClassifyStatus"];

            string clientID = System.Configuration.ConfigurationManager.AppSettings["XL037ID"];
            string AdminID = Needs.Wl.Admin.Plat.AdminPlat.Current.ID;

            List<LambdaExpression> lamdas = new List<LambdaExpression>();
            Expression<Func<PD_PreClassifyProduct, bool>> expression = item => item.PreProduct.AdminID == AdminID &&
                                                                               item.PreProduct.ClientID == clientID &&
            ...
            if (!string.IsNullOrEmpty(classifyStatus))
            {
                int status = Int32.Parse(classifyStatus);
                Expression<Func<Needs.Ccs.Services.Models.PD_PreClassifyProduct, bool>> lambda = item => item.ClassifyStatus == (ClassifyStatus)status;
                lamdas.Add(lambda);
            }
            #endregion
