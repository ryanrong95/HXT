    ...
    public partial class Edit : Uc.PageBase
    {
        ...
        protected void LoadData()
        {
            ...
            this.Model.FileType = EnumUtils.ToDictionary<Needs.Ccs.Services.Enums.FileType>()
                .Where(item => item.Key == Needs.Ccs.Services.Enums.FileType.BusinessLicense.GetHashCode().ToString() ||
                item.Key == Needs.Ccs.Services.Enums.FileType.PowerOfAttorney.GetHashCode().ToString() ||
                item.Key == Needs.Ccs.Services.Enums.FileType.ServiceAgreement.GetHashCode().ToString()).Select(item => new { item.Key, item.Value }).Json();
