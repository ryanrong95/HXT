<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AgreementChangeList.aspx.cs" Inherits="WebApp.Client.ServiceAgreement.AgreementChangeList" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        var AgreementChangeApplyStatus = eval('(<%=this.Model.AgreementChangeApplyStatus%>)');
        ...
        $(function () {
            ...
            $("#AgreementChangeApplyStatus").combobox({
                data: AgreementChangeApplyStatus,
            });
            //默认显示有效状态
            $('#AgreementChangeApplyStatus').combobox('setValue', 3);
        ...
        //费用申请-申请列表初始化
        function loadList() {
            $('#AgreementChangeList').myDatagrid({
                actionName: 'data',
                fitColumns: true,
                queryParams: { AgreementChangeApplyStatus: 3},
                fit: true,
                    ...
                    for (var index = 0; index < data.rows.length; index++) {
                        $('#uploadFile' + index).filebox({
                            ...
                            onChange: function (e) {
                                ...
                                var row = $('#AgreementChangeList').datagrid('getRows')[rowNum];

                                formData.append('RowNum', rowNum);
                                formData.append('ClientID', row.ClientID);
                                formData.append('AgreementChangeApplyID', row.AgreementChangeApplyID);
        ...
        function Search() {
            ...
            var AgreementChangeApplyStatus = $('#AgreementChangeApplyStatus').combobox('getValue');
            ...
            var parm = {
                ...
                AgreementChangeApplyStatus: AgreementChangeApplyStatus
            };

            $('#AgreementChangeList').myDatagrid('search', parm);
        ...
        function ChangeApplyID(val, row, index) {
            return "<a class=\"rowlink\" onclick=\"ViewChangeApply('" + val + "')\" >" + val + "</a>";
        }

        function ViewChangeApply(ID) {
            var url = location.pathname.replace(/AgreementChangeList.aspx/ig, '../AgreementChange/AgreementChangeView.aspx')
                + '?ID=' + ID;
            top.$.myWindow({
                iconCls: "",
                url: url,
        ...
        function IsSAEleUploadOperation(val, row, index) {
            ...
            //if (row.IsSAEleUpload == "0") {
            //    str += '<span style="color:red;">' + row.IsSAEleUploadString + '</span>';
            //} else {
            //    str += '<span style="color:green;">' + row.IsSAEleUploadString + '</span>';
            //}
        ...
        function IsSAPaperUploadOperation(val, row, index) {
            ...
            str += '<a href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:3px; margin-left: 10px;" onclick="SetIsPaperUpload(\''
                + row.AgreementChangeApplyID + '\',\'' + row.ClientID + '\',\'' + row.ClientName + '\',\'' + targetIsSAPaperUpload + '\')" group >' +
                '<span class =\'l-btn-left l-btn-icon-left\'>' +
        ...
        //设置协议变更(纸质)上交状态
        function SetIsPaperUpload(agreementChangeApplyID, clientID, clientName, targetIsSAPaperUpload) {
            var message = '';
            ...
            $.messager.confirm('确认', message, function (r) {
                if (r) {
                    ...
                    $.post(location.pathname + '?action=SetIsPaperUpload', {
                        AgreementChangeApplyID: agreementChangeApplyID,
                        ClientID: clientID,
                        ...
                        if (result.success) {
                            $.messager.alert('提示', result.message, 'info', function () {
                                $('#AgreementChangeList').myDatagrid('reload');
                            });
                        } else {
                            $.messager.alert('提示', result.message, 'info', function () {
                                $('#AgreementChangeList').myDatagrid('reload');
...
<body class="easyui-layout">
    <form id="form1" runat="server" method="post">
            ...
            <div id="search">
                <ul style="margin-top: 5px; margin-bottom: 5px;">
                    <li>
                        ...
                        <input class="easyui-combobox" id="AgreementChangeApplyStatus" data-options="valueField:'Key',textField:'Value',editable:false, width: 180" />
                    </li>
                </ul>
                <ul style="margin-top: 5px; margin-bottom: 5px;">
                    <li>
        ...
        <div id="data" data-options="region:'center',border:false">
            <table id="AgreementChangeList" title="协议变更" data-options="toolbar:'#topBar',">
                <thead>
                    <tr>
                        <th data-options="field:'AgreementChangeApplyID',align:'left',formatter:ChangeApplyID" style="width: 8%;">变更申请编号</th>
                        ...
                        <th data-options="field:'ClientRank',align:'left'" style="width: 6%;">信用等级</th>
                        <th data-options="field:'CreateDate',align:'left'" style="width: 6%;">申请变更日期</th>
                        <th data-options="field:'ClientNature',align:'left'" style="width: 6%;">客户类型</th>
                        <th data-options="field:'ServiceManager',align:'left'" style="width: 6%;">业务员</th>
                        <th data-options="field:'MerchandiserName',align:'left'" style="width: 6%;">跟单员</th>
                        <th data-options="field:'AgreementChangeApplyStatus',align:'left'" style="width: 6%;">状态</th>
                        <th data-options="field:'IsSAEleUpload',align:'left',formatter:IsSAEleUploadOperation" style="width: 10%;">协议变更(电子)</th>
                        <th data-options="field:'IsSAPaperUpload',align:'left',formatter:IsSAPaperUploadOperation" style="width: 10%;">协议变更(纸质)</th>
                    </tr>
                </thead>
            </table>
        </div>
