<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DoneList.aspx.cs" Inherits="WebApp.PvData_PreClassify.DoneList" %>
...
<head runat="server">
    ...
    <%-- <script>
        gvSettings.fatherMenu = '产品预归类(中心数据)';
        gvSettings.menu = '已完成';
    ...
    <script type="text/javascript">
        ...
        function View(index) {
            ...
            if (data) {
                ...
                $.post('?action=GetOrderInfos', { productUnionCode: data.ProductUnionCode }, function (infos) {
                    ...
                    var useType =<%=Needs.Ccs.Services.Enums.PreProductUserType.Pre.GetHashCode()%>;
                    var data2 = {};
                    for (var k in data) {
                        data2[k] = data[k];
                    }
        ...
        function Classify(index) {
            ...
            if (data) {
                MaskUtil.mask();
                $.post('?action=GetOrderInfos', { productUnionCode: data.ProductUnionCode }, function (infos) {
                    var step =<%=Needs.Ccs.Services.Enums.ClassifyStep.PreDoneEdit.GetHashCode()%>;
        ...
        function Operation(val, row, index) {
            ...
            if (row.ClassifyStatusVal == '<%=Needs.Ccs.Services.Enums.ClassifyStatus.Anomaly.GetHashCode()%>') {
                //buttons += '<span style="color: red; font-size: 13px"> | 型号已退回</span>';

                buttons += '<a href="javascript:void(0);" class="easyui-linkbutton" group >' +
                    '<span class =\'l-btn-left l-btn-icon-left\' style="margin-top:3px">' +
                    '<span class="l-btn-text" style="margin-top:3px; color:red">型号已退回</span>' +
                    '<span class="l-btn-icon icon-yg-approvalNopass">&nbsp;</span>' +
                    '</span>' +
                    '</a>';
            ...
            if (row.IsPushStatusWarning) {
                buttons += '<br/>';
                buttons += '<span class="spacialTip" style="margin-left: 2px;">推送失败</span>';
            }
        ...
        function AnomalyOperation(val, row, index) {
            ...
            if (row.ClassifyStatusVal == '<%=Needs.Ccs.Services.Enums.ClassifyStatus.Anomaly.GetHashCode()%>') {
                ...
                buttons += '<a href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:3px" onclick="Return(' + index + ')" group >' +
                    '<span class =\'l-btn-left l-btn-icon-left\'>' +
        ...
        <table id="products" title="已归类产品" data-options="nowrap:false,border:false,fitColumns:false,fit:true,toolbar:'#topBar',">
            <thead>
                <tr>
                    ...
                    <th data-options="field:'Source',align:'center',width:150">来源</th>
                    <%-- <th data-options="field:'CompanyType',align:'center',width:150," style="display:none">公司类型</th>--%>
                    <th data-options="field:'AnomalyBtn',align:'left',width:200,formatter:AnomalyOperation">异常操作</th>
                </tr>
            </thead>
