<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Edit.aspx.cs" Inherits="WebApp.Finance.FundTransferApplies.Edit" %>
    ...
    <script type="text/javascript">
        ...
        function Approve() {
            $.messager.confirm('确认', '同意该资金调拨申请？', function (success) {
                if (success) {
                    MaskUtil.mask();
                    $.post('?action=Approve', { ID: AllData["ID"] }, function (data) {
        ...
        function Deny() {
            $.messager.confirm('确认', '拒绝该资金调拨申请？', function (success) {
                if (success) {
                    MaskUtil.mask();
                    $.post('?action=Deny', { ID: AllData["ID"] }, function (data) {
                        var Result = JSON.parse(data);
                        MaskUtil.unmask();
                        $.messager.alert('提示', Result.info);
                        Close();
        ...
        <form id="form1" runat="server">
            <table id="editTable" style="width: 100%;">
                ...
                <tr>
                    <td class="lbl">备注：</td>
                    <td colspan="3">
                        <input class="easyui-textbox" id="Summary" name="Summary" data-options="required:false,height:70,width:570,validType:'length[1,500]',multiline:true" />
                    </td>
                </tr>
            </table>
        </form>
