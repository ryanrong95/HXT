<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="WebApp.Finance.Receipt.RefundApply.Applicant.List" %>
    ...
    <script>
        ...
        function Operation(val, row, index) {
            ...
            if (row.ApplyStatus == 1) {
                buttons += '<a id="btnDetail" href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:3px" onclick="Cancel(\'' + row.ID + '\')" group >' +
                    '<span class =\'l-btn-left l-btn-icon-left\'>' +
        ...
        //退款申请
        function Cancel(ID) {
            $.messager.confirm('取消退款', '确定要取消这笔退款吗？', function (flag) {
                if (flag) {
                    MaskUtil.mask();
