<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AddDecHead.aspx.cs" Inherits="WebApp.Finance.Declare.AddDecHead" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function openUserCurrentPayApplyList(index, orderID, decHeadID, contrNo, entryID) {
            var url = location.pathname.replace(/AddDecHead.aspx/ig, 'UserCurrentPayApplyList.aspx')
                + "?OrderID=" + orderID
                + "&ContrNo=" + contrNo
                + "&EntryID=" + entryID;
        ...
        //添加报关单
        function AddExecute() {
            var ewindow = $.myWindow.getMyWindow("EditSwapAmount2AddDecHead");
            ...
            //验证是否同币种
            for (var i = 0; i < data.length; i++) {
                if (ewindow.Currency != data[i].Currency) {
                    $.messager.alert('提示', '请勾选币种为' + ewindow.Currency + '的报关单！');
                    return;
                }
            }
            ...
            //这里阻止一下换汇黑名单的银行
            for (var i = 0; i < data.length; i++) {
                if (data[i].SwapSpecialInfo.indexOf(ewindow.bankName) != -1) {
                    $.messager.alert('提示', '选择的报关单中有' + ewindow.bankName + '换汇受限地区！');
                    return;
                }
            }

            for (var i = 0; i < data.length; i++) {
                var oldValue = ewindow.ForAddDecHeadIDs;
                var newValue = '';
                ...
                ewindow.ForAddDecHeadIDs = newValue;
            }

            for (var i = 0; i < data.length; i++) {
                var oldValue = ewindow.ForAddTheInputAmounts;
                var newValue = '';
                if (oldValue == null || oldValue == 'undefined' || oldValue == "") {
                ...
                ewindow.ForAddTheInputAmounts = newValue;
            }
            ...
            <table style="line-height: 30px">
                ...
                <tr>
                    <td colspan="4" class="lbl">
                        <input type="checkbox" id="IsExpssive" name="IsExpssive" class="checkbox checkboxlist" />
                        <label for="IsExpssive">购汇超额</label>
                        <input type="checkbox" id="IsInExpensive" name="IsInExpensive" class="checkbox checkboxlist" />
                        <label for="IsInExpensive" style="margin-left: 25px;">购汇未超额</label>
        ...
        <table id="decheads" data-options="
            ...
            <thead>
                <tr>
                    ...
                    <th data-options="field:'ContrNo',align:'center'" style="width: 10%;">合同号</th>
                    ...
                    <th data-options="field:'SwapStatus',align:'center'" style="width: 5%;">换汇状态</th>
