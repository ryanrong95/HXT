...
{
    public partial class EditSwapAmount : Uc.PageBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                this.Model.BankID = Request.QueryString["BankID"];
                this.Model.BankName = Request.QueryString["BankName"];
            }
        }
        ...
        protected void SubmitApply()
        {
            try
            {
                ...
                swapNotice.Status = Needs.Wl.Models.Enums.SwapStatus.ApprovedAudit;
                swapNotice.ConsignorCode = selectedUnSwapDecHeadList.FirstOrDefault().ConsignorCode;
                ...
                Needs.Ccs.Services.Models.SwapNoticeLog swapNoticeLog = new Needs.Ccs.Services.Models.SwapNoticeLog()
                {
                    ...
                    Status = Needs.Ccs.Services.Enums.SwapStatus.ApprovedAudit,
                    CreateDate = DateTime.Now,
                    Summary = "操作人[" + Needs.Wl.Admin.Plat.AdminPlat.Current.RealName + "]已提交换汇申请，等待审批",
                };
                swapNoticeLog.Enter();

                //根据AEO标志，系统可以自动审批通过，审批人默认张庆永
                if (ConfigurationManager.AppSettings["AEO"] == "false")
                {
                    var admin = ConfigurationManager.AppSettings["ApproveManID"];

                    Needs.Ccs.Services.Models.SwapNoticeLog swapNoticeUpdate = new Needs.Ccs.Services.Models.SwapNoticeLog()
                    {
                        ID = newSwapNoticeID,
                        Admin = admin,
                        CreateDate = DateTime.Now,
                        Status = SwapStatus.Auditing,
                    };
                    swapNoticeUpdate.Update();

                    Needs.Ccs.Services.Models.SwapNoticeLog Log = new Needs.Ccs.Services.Models.SwapNoticeLog()
                    {
                        ID = Guid.NewGuid().ToString("N"),
                        SwapNoticeID = newSwapNoticeID,
                        Admin = admin,
                        ...
                        CreateDate = DateTime.Now,
                        Summary = "系统自动提交了换汇申请审批通过，默认操作人[" + ConfigurationManager.AppSettings["CostApplyManagerName"] + "]，等待换汇",
                    };
                    Log.Enter();
                }
