<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="EditAddDecHead.aspx.cs" Inherits="WebApp.Finance.Swap.EditAddDecHead" %>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    ...
    <script type="text/javascript">
        var CleanIDs = '<%=this.Model.DecHeadIDFromAddDecHeadInEditInfo%>';
        var BankName = '<%=this.Model.BankName1%>';
        var Currency = '<%=this.Model.Currency1%>';
        var SwapNoticeID = '<%=this.Model.SwapNoticeID1%>';
            ...
            $.post('?action=SubmitApply', {
                ...
                if (resultJson.success) {
                    $.messager.alert('消息', resultJson.message, 'info', function () {
                        ...
                        //window.parent.bankID = "";
                        //window.parent.bankName = "";
        ...
        //取消
        function Close() {
            //var ewindow = $.myWindow.getMyWindow("EditInfo2EditAddDecHead");
            //ewindow.cleanIDs = "";
            $.myWindow.close();
        }
        ...
        function RemoveOneRow(decHeadId, contrNo) {
            ...
            $.messager.confirm('确认', '确认要移除该报关单（合同号： <label style=\"color:green\">' + contrNo + '</label> ）吗？', function(r){
                if (r){
                    ...
                    //得到一个新的 CleanIDs， 更新 window.parent.cleanIDs 后，直接刷新该页面
                    var newCleanIDs = "";
                    ...
                    var url = location.pathname
                        + "?BankName=" + BankName
                        + "&DecHeadIDFromAddDecHeadInEditInfo=" + newCleanIDs
                        + "&TheInputAmounts=" + currentTheInputAmounts
                        + "&Currency=" + Currency
                        + '&SwapNoticeID=' + SwapNoticeID;
                    window.location = url;
        ...
        function openUserCurrentPayApplyList(index, orderID, decHeadID, contrNo, entryID) {
            var url = location.pathname.replace(/EditAddDecHead.aspx/ig, '../../Declare/UserCurrentPayApplyList.aspx')
                + "?OrderID=" + orderID
                + "&ContrNo=" + contrNo
                + "&EntryID=" + entryID;
