<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SwapedList.aspx.cs" Inherits="WebApp.Finance.Swap.SwapList" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        //查看订单
        function View(id) {
            var url = location.pathname.replace(/SwapedList.aspx/ig, 'Details.aspx?ID=' + id);
            top.$.myWindow({
                iconCls: "",
                url: url,
                noheader: false,
        ...
        function BillDetail(id) {
            var url = location.pathname.replace(/SwapedList.aspx/ig, 'BillDetails.aspx?ID=' + id);
            $.myWindow({
                iconCls: "",
                url: url,
                noheader: false,
        ...
        function DownloadBillDetail(id) {
            MaskUtil.mask();
            $.post('?action=DownloadBillDetail', { SwapNoticeID: id }, function (result) {
                MaskUtil.unmask();
        ...
        function Operation(val, row, index) {
            ...
            buttons += '<a id="btnDetail" href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:3px" onclick="View(\'' + row.ID + '\')" group >' +
                ...
                '</span>' +
                '</a>';
            buttons += '<a id="btnDetail" href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:3px" onclick="BillDetail(\'' + row.ID + '\')" group >' +
                '<span class =\'l-btn-left l-btn-icon-left\'>' +
                '<span class="l-btn-text">做账明细</span>' +
                '<span class="l-btn-icon icon-search">&nbsp;</span>' +
                '</span>' +
                '</a>';
            buttons += '<a id="btnDetail" href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:3px" onclick="DownloadBillDetail(\'' + row.ID + '\')" group >' +
                '<span class =\'l-btn-left l-btn-icon-left\'>' +
                '<span class="l-btn-text">导出做账明细</span>' +
...
<body class="easyui-layout">
    ...
    <div id="data" data-options="region:'center',border:false">
        <table id="datagrid" title="换汇通知-已换汇" data-options="fitColumns:true,fit:true,rownumbers:true,singleSelect:false,toolbar:'#topBar'">
            <thead>
                <tr>
                    ...
                    <th data-options="field:'UseSwapNoticeAmountCNY',align:'left'" style="width: 6%;">已使用</th>
                    ...
                    <th data-options="field:'Btn',align:'center',formatter:Operation" style="width: 370px;">操作</th>
                </tr>
            </thead>
        </table>
    </div>
