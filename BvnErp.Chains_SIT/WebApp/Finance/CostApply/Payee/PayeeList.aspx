<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PayeeList.aspx.cs" Inherits="WebApp.Finance.CostApply.PayeeList" %>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    ...
    <script type="text/javascript">
        ...
        function EditPayee(CostApplyPayeeID) {
            var url = location.pathname.replace(/PayeeList.aspx/ig, './EditPayee.aspx') + '?From=Edit&CostApplyPayeeID=' + CostApplyPayeeID;
        ...
        function DeletePayee(index) {
            ...
            tip += '<label>收款方名称：' + row.PayeeName + '</label><br>';
            tip += '<label>收款方账号：' + row.PayeeAccount + '</label><br>';
            tip += '<label>收款方银行：' + row.PayeeBank + '</label>';
                ...
                buttons: [{
                    ...
                    handler: function () {
                        ...
                        $.post(location.pathname + '?action=Delete', {                            
                            CostApplyPayeeID: row.CostApplyPayeeID,
                        }, function (res) {
                            MaskUtil.unmask();
        ...
        function Select() {
            ...
            ewindow.SelectPayee.PayeeName = selectPayee[0].PayeeName;
            ewindow.SelectPayee.PayeeAccount = selectPayee[0].PayeeAccount;
            ewindow.SelectPayee.PayeeBank = selectPayee[0].PayeeBank;

            ewindow.$("#PayeeName").textbox('setValue', selectPayee[0].PayeeName);
            ewindow.$("#PayeeAccount").textbox('setValue', selectPayee[0].PayeeAccount);
            ewindow.$("#PayeeBank").textbox('setValue', selectPayee[0].PayeeBank);
        ...
        <table id="PayeeList" data-options="fitColumns:true,fit:true,border:false,toolbar:'#topBar',">
            <thead>
                <tr>
                    ...
                    <th data-options="field:'PayeeName',align:'left'" style="width: 37%;">收款方名称</th>
                    <% } %>
                    <% else %>
                    <% { %>
                    <th data-options="field:'PayeeName',align:'left'" style="width: 40%;">收款方名称</th>
                    <% } %>
                    
                    <th data-options="field:'PayeeAccount',align:'left'" style="width: 26%;">收款方账号</th>
                    <th data-options="field:'PayeeBank',align:'left'" style="width: 16%;">收款方银行</th>
