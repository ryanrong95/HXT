<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="UnProcessList.aspx.cs" Inherits="WebApp.Classify.Outside.BeforeDecChangeList" %>
   ...
   <%-- <script>
    ...
    <script>
        ...
        function ReClassify(index) {
            ...
            if (rowdata) {
                MaskUtil.mask();
                $.post('?action=Lock', { ID: rowdata.OrderItemID }, function (res) {
                    MaskUtil.unmask();
                var result = JSON.parse(res);
                if (result.success) {
                    var url = location.pathname.replace(/UnProcessList.aspx/ig, '../Product/Edit.aspx') + '?ID=' + rowdata.OrderItemID + '&From=<%=Needs.Ccs.Services.Enums.ClassifyStep.ReClassify.GetHashCode()%>'
...
            });
        ...
        function UnLock(index) {
            ...
            if (rowdata) {
                MaskUtil.mask();
                $.post('?action=UnLock', { ID: rowdata.OrderItemID }, function (res) {
                    MaskUtil.unmask();
                    var result = JSON.parse(res);
                    if (result.success) {
                        $.messager.alert('提示', result.message);
