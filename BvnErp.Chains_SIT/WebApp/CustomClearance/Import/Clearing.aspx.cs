...
namespace WebApp.OrderWaybill
{
    public partial class Clearing : Uc.PageBase
    {
        ...
        protected void data()
        {
            ...
            //    .Where(item => item.HKDeclareStatus == HKDeclareStatus.UnDeclare);

            var data = new Needs.Ccs.Services.Views.HKClearingDataNew().OrderByDescending(x => x.ArrivalDate).ThenBy(x => x.Carrier.Code).AsQueryable();
        ...
        protected void Declared()
        {
            try
            {
                ...
                var datas = new Needs.Ccs.Services.Views.OrderWaybillOrigin().Where(item => IDs.Contains(item.ID));
