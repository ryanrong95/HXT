...
{
    public partial class Clearing : Uc.PageBase
    {
        ...
        protected void data()
        {
            ...
            var manifest = Needs.Wl.Admin.Plat.AdminPlat.Current.Warehouse.OutputWayBill.OrderByDescending(x => x.Voyage.LoadingDate).ThenBy(x => x.Voyage.ID)
               .Where(item => item.Voyage.HKDeclareStatus == false).AsQueryable();
        ...
        protected void Export()
        {
            ...
            //var manifest = Needs.Wl.Admin.Plat.AdminPlat.Current.Warehouse.OutputWayBill
            //   .Where(item => item.Voyage.HKDeclareStatus == HKDeclareStatus.UnDeclare).AsQueryable();
            //if (!string.IsNullOrEmpty(VoyageNo))
            //{
            //    VoyageNo = VoyageNo.Trim();
            //    manifest = manifest.Where(item => item.Voyage.ID.Contains(VoyageNo));
            ...
            //if (!string.IsNullOrEmpty(StartTime))
            //{
            //    manifest = manifest.Where(item => item.Voyage.LoadingDate.Value.CompareTo(StartTime) >= 0);
            //}
            //if (!string.IsNullOrEmpty(EndTime))
            //{
            //    manifest = manifest.Where(item => item.Voyage.LoadingDate.Value.CompareTo(EndTime) <= 0);
            //}
            ...
            //    BillNo = item.ID,
            //    HKLicense = item.Voyage.HKLicense,
            //    LoadingDate = item.Voyage.LoadingDate.Value.ToString("yyyy-MM-dd"),
            //    CarrierName = item.Voyage.Carrier.Name,
            //    ProductQty = item.PackNum,
            //    TotalPrice = item.GoodsValue,
            //    Currency = item.Currency,
            //    Status = item.Voyage.HKDeclareStatus.GetDescription(),
            //};
            ////数据源
            ...
            //DataTable table = NPOIHelper.JsonToDataTable(manifest.Select(convert).ToArray().Json());
            //var NPOI = NPOIContext.Current;
            //NPOI.SetColumns(columns);
            //NPOI.DataSource = table;
            //NPOI.WriteToExcel();
            //string fileName = DateTime.Now.Ticks + "香港出口清关" + ".xlsx";
        ...
        protected void Declared()
        {
            try
            {
                ...
                var datas = Needs.Wl.Admin.Plat.AdminPlat.Current.Warehouse.OutputWayBill.Where(item => IDs.Contains(item.ID));
                foreach (var data in datas)
                {
                    //清关
                    data.Voyage.Clear();
                }
                Response.Write((new { success = true, message = "清关成功" }).Json());
            }
            catch (Exception ex)
            {
                Response.Write((new { success = false, message = "清关失败：" + ex.Message }).Json());
            }
        }
    }
}