<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ExpressBill.aspx.cs" Inherits="WebApp.SZWareHouse.Exit.ExpressBill" %>
...
<head runat="server">
    ...
    <script>
        ...
        function InitExitNotice() {
            ...
            $("#billStamp").attr("src", ExitNotice.BillStamp);
        };
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        ...
                        //div.appendChild(br);
                        ////添加子模板面单
                        //if (data.SubPrintTemplates != null) {
                        //    for (var i = 0; i < data.SubPrintTemplates.length; i++) {
                        //        var divson = document.createElement("div");
                        //        divson.innerHTML = data.SubPrintTemplates[i];
                        //        div.appendChild(divson);
                        //    }
                        //}
                        var cl = data.ShipperCode == "KYSY" ? "" : "style='height: 800px;position: relative;'";
                        var $div1 = $("#expresskdd");
                        var $divf = $("<div " + cl +"></div>").append(data.PrintTemplate);
                        $div1.append($divf);
                        //添加子模板面单
                        if (data.SubPrintTemplates != null) {
                            for (var i = 0; i < data.SubPrintTemplates.length; i++) {
                                var $divc = $("<div " + cl +"></div>");
                                $divc.append(data.SubPrintTemplates[i]);
                                $div1.append($divc);
                            }
                        }


                        if (data.ShipperCode == "KYSY") {
                            $(".print_paper:eq(6)").css({ "max-height": 25 });
                            $(".print_paper:eq(11)").css({ "max-height": 53 });
                            $(".print_paper:eq(16)").css({ "max-height": 34 });
                            $(".print_paper:eq(6) div").css({ "max-height": 30 });
                            $(".print_paper:eq(11) div").css({ "max-height": 53 });
                            $(".print_paper:eq(16) div").css({ "max-height": 34 });
                            $(".no_border:eq(5) td").css({ "min-width": 60 });
                        }
                        hideKdd();
                    } else {
        ...
        function hideKdd() {
            if (ExitStatus == 4) {
                $(".hideKdd").css('display', 'none');
            } else {
                $(".hideKdd").css('display', 'inline-table');
            }
        }
        ...
        <div title="送货单" style="padding: 10px;">
            <form id="form1">
                <div style="margin-top: 10px; margin-bottom: 10px;">
                    ...
                    <a href="javascript:void(0);" class="easyui-linkbutton hideKdd" id="printkdd"
                ...
                <div id="container" style="width: 745px; margin: 20px auto">
                    ...
                    <div class="print-row">
                        ...
                        <div style="float: left; position: relative; left: 100px; top: 1px;">
                            <img id="billStamp" style="width: 120px;" />
                        </div>
                    </div>
                </div>
            </form>
            ...
            <div id="expresskdd" style="margin: 0px">
                    <style>
                    #expresskdd * {
