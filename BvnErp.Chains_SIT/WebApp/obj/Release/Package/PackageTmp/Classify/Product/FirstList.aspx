<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FirstList.aspx.cs" Inherits="WebApp.Classify.Product.FirstList" %>
        ...
        function Classify(index) {
            ...
            if (rowdata) {
                ...
                $.post('?action=Lock', { ID: rowdata.ID }, function (res) {
                    ...
                    var result = JSON.parse(res);
                    if (result.success) {
                        var url = location.pathname.replace(/FirstList.aspx/ig, 'Edit.aspx') + '?ID=' + rowdata.ID + '&From=<%=Needs.Ccs.Services.Enums.ClassifyStep.Step1.GetHashCode()%>'
