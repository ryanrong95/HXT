...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            //显示发票扫码页面二维码
            var imgSrc = '?action=GetWxInvoicePageQrCode&&InvoiceNoticeID=' + getQueryString("ID");
            $("#invoicePageQrCode").append('<img height="120" width="120" src="' + imgSrc + '" alt="发票录入页面二维码" />');

            // 连续加载发票信息
            continueLoadInvoiceData();
        });
        ...
        function ConfirmInvoice() {
            ...
            ShowInvoice(InvoiceNoticeID, function () {
        ...
        function ShowInvoice(InvoiceNoticeID, callback) {
            $.post('?action=InvoiceData', {
                ID: InvoiceNoticeID,
            }, function (res) {
                var resJson = JSON.parse(res);
                if (resJson.success) {
                    $("#com2-dialog-content tbody").html('');
                    var info = "";
                    if (resJson.data.length > 0) {
                        for (var i = 0; i < resJson.data.length; i++) {
                            info += '<tr>';
                            info += '<td>' + resJson.data[i].InvoiceCode + '</td>';
                            info += '<td>' + resJson.data[i].InvoiceNo + '</td>';
                            info += '<td>' + resJson.data[i].InvoiceDate + '</td>';
                            info += '<td>' + resJson.data[i].Amount + '</td>';
                            info += '<td>' + resJson.data[i].InvoiceTypeName + '</td>';
                            info += '</tr>';
                        }
                    } else {
                        info += '<tr><td colspan="10" style="text-align: center;">暂无数据</td></tr>';
                    }

                    $("#com2-dialog-content tbody").html(info);
        ...
        function PrintWaybill() {
            ...
            $.post('?action=InvoiceData', {
                ...
                var resJson = JSON.parse(res);
                if (!resJson.success) {
                    $.messager.alert('提示', "请求发票信息错误");
                    return;
                }

                if (resJson.data.length <= 0) {
                    $.messager.alert('提示', "请录入发票信息");
                    return;
                }
        ...
        // 连续加载发票信息
        function continueLoadInvoiceData() {
            $.post('?action=InvoiceData', {
                ID: getQueryString("ID"),
            }, function (res) {
                var resJson = JSON.parse(res);
                if (resJson.success) {
                    $("#invoiceDataTable tbody").html('');
                    var info = "";
                    if (resJson.data.length > 0) {
                        for (var i = 0; i < resJson.data.length; i++) {
                            info += '<tr>';
                    ...
                    $("#invoiceDataTable tbody").html(info);

                    setTimeout(continueLoadInvoiceData, 2000);
                }
            });
        }
    ...
    <style type="text/css">
        ...
        #invoiceDataTable, #com2-dialog-content table {
            border-collapse: collapse;
            margin: 5px 0;
        }

            #invoiceDataTable, #invoiceDataTable td, #invoiceDataTable th,
            #com2-dialog-content table, #com2-dialog-content table td, #com2-dialog-content table th {
...
<body class="easyui-layout" style="overflow-y: scroll;">
    <div id="tt" class="easyui-tabs" style="width: auto;">
        <div title="开票确认" style="display: none; padding: 5px;">
            ...
            <div data-options="region:'center',border: false," style="width: 48%; float: left; margin-top: 5px;">
                <div class="sec-container">
                    <div id="panel3" class="easyui-panel" title="其它信息" style="height: 247px;">
                        <div class="sub-container">
                            <table class="row-info" style="width: 100%;" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td>含税金额：</td>
            ...
            <div data-options="region:'center',border: false," style="width: 51%; float: left; margin-top: 5px;">
                <div data-options="region:'center',border: false,">
                    <div class="sec-container">
                        <div id="panel4" class="easyui-panel" title="发票信息">
                            <div style="float: left; width: 150px; margin-left: 5px; position: absolute;">
                                <div>
                                    <h4>录入发票</h4>
                                    <h4>请用<span style="color: limegreen;">微信</span>扫描二维码</h4>
                                </div>
                                <div id="invoicePageQrCode" style="margin-top: 5px;"></div>
                            </div>
                            <div id="invoiceDataDiv" style="float: left; height: 220px; width: 800px; padding-left: 180px;">
                                <table id="invoiceDataTable" class="invoice-table">
                                    <thead>
                                        <tr>
                                            <th style="width: 150px;">发票代码</th>
                                            <th style="width: 150px;">发票号码</th>
                                            <th style="width: 120px;">开票日期</th>
                                            <th style="width: 150px;">金额(不含税)</th>
                                            <th style="width: 180px;">发票类型</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    </tbody>
    ...
    <div id="com2-dialog" class="easyui-dialog" data-options="resizable:false, modal:true, closed: true, closable: false,">
        <div id="com2-dialog-content" style="margin: 15px 15px 15px 15px;">
