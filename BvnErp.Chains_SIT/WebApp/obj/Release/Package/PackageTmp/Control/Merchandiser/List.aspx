<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="WebApp.Control.Merchandiser.List" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function Auditing(index) {
            ...
            if (rowdata) {
                ...
                if (rowdata.ControlTypeValue == '<%=Needs.Ccs.Services.Enums.OrderControlType.CCC.GetHashCode()%>' ||
                    ...
                    url = location.pathname.replace(/List.aspx/ig, 'ExceedLimitDisplay.aspx') + '?ID=' + rowdata.ID + '&OrderID=' + rowdata.OrderID + '&Ststus=View';
        ...
        function Operation(val, row, index) {
            var buttons = '';
            if (row.ControlStatus == '待审批' || row.ControlStatus == '待总部审批')
                if (row.ControlTypeValue != '<%=Needs.Ccs.Services.Enums.OrderControlType.ExceedLimit.GetHashCode()%>'
                    && row.ControlTypeValue != '<%=Needs.Ccs.Services.Enums.OrderControlType.OverdueAdvancePayment.GetHashCode()%>') {
                ...
                else if (row.ControlTypeValue == '<%=Needs.Ccs.Services.Enums.OrderControlType.ExceedLimit.GetHashCode()%>' ||
                    row.ControlTypeValue == '<%=Needs.Ccs.Services.Enums.OrderControlType.OverdueAdvancePayment.GetHashCode()%>') {
                    buttons += '<a href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:3px" onclick="Auditing(' + index + ')" group >' +
                        '<span class =\'l-btn-left l-btn-icon-left\'>' +
