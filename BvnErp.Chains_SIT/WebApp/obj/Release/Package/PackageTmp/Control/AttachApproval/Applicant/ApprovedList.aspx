<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ApprovedList.aspx.cs" Inherits="WebApp.Control.AttachApproval.Applicant.ApprovedList" %>
    ...
    <script type="text/javascript">
        ...
        function showApproveDetailWindow(orderControlStepID, controlTypeInt) {
            ...
            if (controlTypeInt == '<%=Needs.Ccs.Services.Enums.OrderControlType.GenerateBillApproval.GetHashCode()%>') {
                url = location.pathname.replace(/ApprovedList.aspx/ig, '../Approver/ApproveGenerateBill.aspx')
                ...
                url = location.pathname.replace(/ApprovedList.aspx/ig, '../Approver/ApproveDeleteModel.aspx')
                    + '?OrderControlStepID=' + orderControlStepID
                    + '&From=Applicant';
                width = 1600;
                height = 650;
            } else if (controlTypeInt == '<%=Needs.Ccs.Services.Enums.OrderControlType.ChangeQuantityApproval.GetHashCode()%>') {
                url = location.pathname.replace(/ApprovedList.aspx/ig, '../Approver/ApproveChangeQuantity.aspx')
                    + '?OrderControlStepID=' + orderControlStepID
                    + '&From=Applicant';
                width = 1600;
                height = 650;
            } else if (controlTypeInt == '<%=Needs.Ccs.Services.Enums.OrderControlType.SplitOrderApproval.GetHashCode()%>') {
                url = location.pathname.replace(/ApprovedList.aspx/ig, '../Approver/ApproveSplitOrder.aspx')
                    + '?OrderControlStepID=' + orderControlStepID
                    + '&From=Applicant';
                width = 1600;
                height = 650;
