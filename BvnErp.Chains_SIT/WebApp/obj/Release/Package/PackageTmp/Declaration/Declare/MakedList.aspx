<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MakedList.aspx.cs" Inherits="WebApp.Declaration.Declare.MakedList" %>
    ...
    <script type="text/javascript">
        ...
        function Search() {
            ...
            $('#datagraid').myDatagrid('search', {
                ContrNo: ContrNo, OrderID: OrderID, SeqNo: SeqNo, BaseCusReceiptCode: BaseCusReceiptCode, StartDate: StartDate, EndDate: EndDate, VoyageID: VoyageID, VoyageType: VoyageType, ClientName: ClientName,
                EntryID: EntryID, DecHeadSpecialType: JSON.stringify(DecHeadSpecialTypeArray),
            });
        }
        ...
        function ReceiptHead(id, ContrNo) {
            var url = location.pathname.replace(/MakedList.aspx/ig, 'DecTrace.aspx?ID=' + id + '&ContrNo=' + ContrNo);
            top.$.myWindow({
                iconCls: "",
                url: url,
                noheader: false,
        ...
        function SearchHead(id, OrderID) {
            var url = location.pathname.replace(/MakedList.aspx/ig, 'Declare.aspx?ID=' + id + '&Source=View&SourcePage=Maked&OrderID=' + OrderID);
            window.location = url;
        }

        function EditHead(id, OrderID) {
            var url = location.pathname.replace(/MakedList.aspx/ig, 'Declare.aspx?ID=' + id + '&Source=Edit&SourcePage=Maked&OrderID=' + OrderID);
            window.location = url;
        }

        //重新发送报文
        function ResendMsg(id) {
            $.messager.confirm('确认', "是否重新发送报关单数据至单一窗口？", function (success) {
                if (success) {
                    MaskUtil.mask();//遮挡层
                    $.post('?action=ResendMsg', { ID: id }, function (res) {
        ...
        function Operation(val, row, index) {
            ...
            // 为了让发单人自己修改报关数据，允许始终编辑
            //if (row["Status"] == "02" || row["Status"] == "03" || row["Status"] == "S0") {
            ...
            buttons +=
                ...
                '<span class =\'l-btn-left l-btn-icon-left\'>' +
                '<span class="l-btn-text">核对单</span>' +
            ...
            if (row["IsDecHeadFile"] == true) {
                buttons += '<a id="btnView" name="btnView" href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" data-fileurl="' + row.URL + '" style="margin:1px; height: 26px;" group >' +
                    //'<span class =\'l-btn-left l-btn-icon-left\'>' +
                    '<span class="l-btn-text" style="margin-top: -3px;">报关单</span>' +
                    //'<span class="l-btn-icon icon-search">&nbsp;</span>' +
                    '</span>' +
                    '</a>';
            }

            //ryan 暂存失败，允许页面点击重新发生按钮
            if (row["Status"] == "F1") {
                buttons += '<a href="javascript:void(0);" class="easyui-linkbutton l-btn l-btn-small" style="margin:1px" onclick="ResendMsg(\'' + row.ID + '\')" group >' +
                    '<span class =\'l-btn-left l-btn-icon-left\'>' +
                    '<span class="l-btn-text">重发</span>' +
        ...
        function AutoOut() {
            ...
            $.messager.confirm('确认', "此步骤暂用，不是实际流程", function (success) {
                if (success) {
                    MaskUtil.mask();//遮挡层
                    $.post('?action=AutoOut', { ID: id }, function (res) {
                        MaskUtil.unmask();//关闭遮挡层
                        var result = JSON.parse(res);
                        $.messager.alert('消息', result.message, function (r) {
                            Search();
        ...
        //Icgoo提交完税价格
        function IcgooPostDutiablePrice() {
            var ids = [];
            var message = "";
            var rows = $('#datagraid').datagrid('getChecked');
            if (rows.length < 1) {
            ...
            $.messager.confirm('确认', "此步骤暂用，不是实际流程", function (success) {
                if (success) {
                    MaskUtil.mask();//遮挡层
                    $.post('?action=IcgooPostDutiablePrice', { ID: id }, function (res) {
                        MaskUtil.unmask();//关闭遮挡层
                        var result = JSON.parse(res);
                        $.messager.alert('消息', result.message, function (r) {
                            Search();
        ...
        function Check(ID) {
            var url = location.pathname.replace(/MakedList.aspx/ig, 'DecListPrint.aspx?ID=' + ID);
            $.myWindow({
                iconCls: "",
                url: url,
                noheader: false,
        ...
        function CheckHead(ID) {
            var url = location.pathname.replace(/MakedList.aspx/ig, 'DecHeadPrint.aspx?ID=' + ID);
            $.myWindow({
                iconCls: "",
                url: url,
                noheader: false,
        ...
        function MakerCheck(id, orderID, VoyageID) {
            var url = location.pathname.replace(/MakedList.aspx/ig, 'DecCheck.aspx?ID=' + id + '&VoyageID=' + VoyageID + '&OrderID=' + orderID + '&Form=Maked');
            $.myWindow({
                iconCls: "",
                url: url,
                noheader: false,
        ...
        function DownloadDeclare() {
            var StartDate = $('#StartDate').datebox('getValue');
            var EndDate = $('#EndDate').datebox('getValue');
            $.post('?action=DownloadDeclare', { StartDate: StartDate, EndDate: EndDate }, function (data) {
...
<body class="easyui-layout">
    <div id="topBar">
        <div id="tool">
            ...
            <a id="btnSucceed" href="javascript:void(0);" style="display: none;" class="easyui-linkbutton" data-options="iconCls:'icon-ok'" onclick="AutoOut()">一键出库</a>
            <%--   <a id="btnPost" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-ok'" onclick="IcgooPostDutiablePrice()">Icgoo提交</a>--%>
        </div>
        <div id="search">
            <ul>
                ...
                <li>
                    ...
                    <a id="btnDownload" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-save'" onclick="DownloadDeclare()">报关清单</a>
    ...
    <div id="data" data-options="region:'center',border:false">
        <table id="datagraid" title="已申报" data-options="fitColumns:true,border:false,fit:true,singleSelect:true,nowrap:false,toolbar:'#topBar'">
            <thead data-options="frozen:true">
                <tr>
                    <th data-options="field:'CheckBox',align:'center',checkbox:true" style="width: 20px">全选</th>
                    <th data-options="field:'ContrNO',align:'left'" style="width: 7%">合同号</th>
                    ...
                    <th data-options="field:'CreateDeclareAdminName',align:'center'" style="width: 4%">制单员</th>
                    <th data-options="field:'CustomSubmiterAdminName',align:'center'" style="width: 5%">录入及申报员</th>
                    <th data-options="field:'Btn',align:'left',formatter:Operation" style="width: 19%">操作</th>
                </tr>
            </thead>
            <thead>
                <tr>
                    ...
                    <th data-options="field:'GuaranteeNo',align:'left'" style="width: 7%">保函号</th>
                    ...
                    <th data-options="field:'DDate',align:'left'" style="width: 7%">申报日期</th>
