<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Edit.aspx.cs" Inherits="WebApp.Order.Delivery.Edit" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('#ExpressCompany').combobox({
                ...
                onSelect: function (record) {
                    //非顺丰、跨越速运，只能到付
                    if (record.Name != '顺丰' && record.Name != '跨越速运' && record.Name != 'EMS') {
                        $('#PayType').combobox('setValue', '<%=Needs.Ccs.Services.Enums.PayType.CollectPay.GetHashCode()%>');
                        $("#PayType").combobox('readonly', true);
                    }
                    else {
                        if (record.Name == "EMS") {
                            var payTypeEms = eval('([{"Key":"1","Value":"寄付"},{"Key":"2","Value":"到付"}])');
                            InitPayType(payTypeEms);
                        } else {
                            InitPayType(payType);
                        }
        ...
        function InitPayType(payType) {

            $('[name="ThirdPartyShow"]').hide();
            //初始化支付方式下拉框
            $('#PayType').combobox({
                data: payType,
                onSelect: function (record) {
                    //第三方月结时， 显示月结号输入框，必填
                    if (record.Value == '第三方月结') {
                        $('[name="ThirdPartyShow"]').show();
                        //$('#ThirdPartyCardNo').show();
                        $('#ThirdPartyCardNo').textbox('textbox').validatebox('options').required = true;
                    }
                    else {
                        $('[name="ThirdPartyShow"]').hide();
                        //$('#ThirdPartyCardNo').hide();
                        $('#ThirdPartyCardNo').textbox('textbox').validatebox('options').required = false;
                    }
                }
            });
        }
        ...
        <form id="form1" runat="server" method="post">
            <table id="SZDelivery" class="radioTable" style="font-size: 12px">
                ...
                <tr id="PayTypeTR">
                    ...
                    <td name="ThirdPartyShow">月结号：</td>
                    <td name="ThirdPartyShow">
                        <input class="easyui-textbox" id="ThirdPartyCardNo" name="ThirdPartyCardNo" style="width: 300px;" />
                    </td>
                </tr>
            </table>
