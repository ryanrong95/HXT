<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Display.aspx.cs" Inherits="WebApp.Order.Delivery.Display" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $.post('?action=isCanDelivery', { MainOrderID: order['ID'], OrderID: order['OrderID'] , ClientID: order['ClientID'] }, function (data) {
                var Result = JSON.parse(data);
                if (!Result.success) {
                    $("#btnAdd").show();
                    $("#showMsg").hide();
                }
                if (!Result.OverDuePayment) {
                    $("#showOverDuePaymentMsg").hide();
                }
                else {
                    $("#btnAdd").hide();
                    document.getElementById('orderId').innerText = Result.GetOrderId;
                }
        ...
        function Operation(val, row, index) {
            if (row.CenterExeStatus == '<%=Needs.Ccs.Services.Models.CgPickingExcuteStatus.PartialShiped.GetHashCode()%>'
                || row.CenterExeStatus == '<%=Needs.Ccs.Services.Models.CgPickingExcuteStatus.Completed.GetHashCode()%>'
...
<body>
    ...
    <div id="topBar">
        <div id="tool">
            ...
            <span id="showMsg" style="color: red">客户货款或税代费超过垫款上限</span>
            <span id="showOverDuePaymentMsg" style="color: red">客户存在垫款超期,超期订单号：<label id="orderId" class="label" /></span>
        </div>
        <div id="search">
            <ul>
                <li>
        ...
        <table id="deliveries" title="送货信息">
            <thead>
                <tr>
                    ...
                    <th data-options="field:'CenterExeStatusName',align:'center'" style="width: 15%">送货状态</th>
