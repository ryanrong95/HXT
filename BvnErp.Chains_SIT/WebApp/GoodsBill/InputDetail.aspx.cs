...
{
    public partial class InputDetail : Uc.PageBase
    {
        ...
        protected void data()
        {
            ...
            string DDateFrom = Request.QueryString["DDateFrom"];
            string DDateTo = Request.QueryString["DDateTo"];
            string VoyNo = Request.QueryString["VoyNo"];

            List<LambdaExpression> lamdas = new List<LambdaExpression>();

            using (var query = new Needs.Ccs.Services.Views.InputViewSpeed())
            {
                var view = query;
                ...
                if (!string.IsNullOrWhiteSpace(DDateFrom))
                {
                    DDateFrom = DDateFrom.Trim();
                    DateTime dtFrom = Convert.ToDateTime(DDateFrom);
                    view = view.SearchByDFrom(dtFrom);
                }


                if (!string.IsNullOrWhiteSpace(DDateTo))
                {
                    DDateTo = DDateTo.Trim();
                    DateTime dtTo = Convert.ToDateTime(DDateTo).AddDays(1);
                    view = view.SearchByDTo(dtTo);
                }
        ...
        protected void Download()
        {
            ...
            else
            {
                ...
                foreach(var item in InputView)
                {
                    ...
                    dr["GoodsStatus"] = item.GoodsStatus;
                    dr["GName"] = item.GName;
                    dr["GoodsBrand"] = item.GoodsBrand;
                    dr["GoodsModel"] = item.GoodsModel;
                    dr["GQty"] = item.GQty;
                    dr["Gunit"] = item.Gunit;
                    dr["DeclPrice"] = item.DeclPrice;
                ...
                try
                {
                    ...
                    excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "GoodsStatus", ExcelColumn = "货物状态", Alignment = "center" });
                    ...
                    excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "ExciseTaxPayed", ExcelColumn = "六联单号", Alignment = "center" });
                    //调用导出方法
